...
- The source pool sets `default_transaction_read_only=on` at connect time (T-EXTRACT), so a statement issued outside an explicit transaction, such as the `system_identifier` read, is read-only too; before that change only statements inside the holder transaction were.
