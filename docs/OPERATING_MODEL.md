...
GitHub is the source of truth for a task that is open, in progress or blocked (T-0196): `tools/tracker.py` runs the same subcommands as before (`new`, `start`, `log`, `block`, `move`, `cancel`, `close --outcome --postmortem`, `list`, `board`, `validate`) over the `gh` CLI, plus `show T-NNNN`, which replaces reading `tracker/tasks/T-NNNN-*.md` for open work — that file no longer exists once a task is on GitHub. `tools/tracker.py migrate [--dry-run]` was the one-time move; only the orchestrator runs a mutating command. Agents return structured results that include a one-line post-mortem; the orchestrator records it on close. `tracker/tasks/` is now a read-only archive of closed and cancelled history, written once, at the moment a task closes, and never edited again; `tracker/epics/` is unaffected. `tracker/BOARD.md` is regenerated from GitHub plus the archive on every mutating command, so the repository keeps an offline, versioned snapshot, and says at the top that it is generated and when. A task is never closed without a post-mortem; a cancelled task carries its reason. If `gh` is missing, unauthenticated or the network is down, a mutating command changes nothing and says which; `list` and `show` fall back to the archive plus the last `tracker/BOARD.md` and say they did.
