...
- Strong at "long-horizon agentic work, knowledge work, vision, and memory tasks"; it "performs well out of the box" on 4.7 prompts.
...
- **Better at finding bugs**, yet an old harness can *measure* lower recall because 4.8 obeys "don't nitpick" — "a harness effect, not a capability regression".
- **More regular, higher-quality progress updates** in long agentic traces.
- **Direct, opinionated prose;** a persistent cream/serif design default that "will feel off for dashboards, dev tools".

## Prompt skeleton

Per [p48]: "specify the task, intent, and relevant constraints upfront in the
first human turn". Tags are our container; the page prescribes no markup.

```text
<role>Who the model is for this task.</role>
<context>Repo, phase, the files that matter.</context>
<task>The complete spec, first turn, no follow-up needed.</task>
<constraints>Scope stated per item, never implied ("every section, not
just the first one"). Files you may write; what not to touch.</constraints>
...
Every implicit "and the rest" belongs in `<constraints>`, and a vague bar is
obeyed as written. The page's model of a concrete one: "report any
...
- Depth at low effort: "This task involves multistep reasoning. Think carefully through the problem before responding."
...
- **Qualitative bars in review prompts** — "be conservative", "don't nitpick". They now get followed.
- **Generic design negations** ("don't use cream"), which "shift the model to a different fixed palette rather than producing variety". Give a concrete spec.
...
Control length by prompt, not by starving effort.

## For coding agents

- **Effort:** start at `xhigh`; `xhigh` or `high` for interactive products.
- **Autonomy over turns:** it "reasons more after user turns", so "add autonomous features like an auto mode, and reduce the number of human interactions".
...
- **Review stages:** split finding from filtering; at the finding stage the job "is coverage rather than filtering". Iterate "against a subset of your evals".

**Unverified:** the page says nothing about stopping early, tests as verification,
or file-edit mechanics. Our rules there are general practice, not 4.8 guidance.
