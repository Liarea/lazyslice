...
A change under `mask/` needs its own `mask/vX.Y.Z` tag, and `go.mod`'s `require github.com/Liarea/lazyslice/mask` bumped to it, landed before the tool's own tag (T-0285; `mask/CLAUDE.md` has the full sequence). `make tag TAG=mask/vX.Y.Z` cuts that tag the same way `make tag TAG=vX.Y.Z` cuts the tool's.
