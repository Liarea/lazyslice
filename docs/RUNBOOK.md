...
A change under `mask/` needs its own `mask/vX.Y.Z` tag, and `go.mod`'s `require github.com/Liarea/lazyslice/mask` bumped to it, landed before the tool's own tag (T-0285; `mask/CLAUDE.md` has the full sequence). `make tag TAG=mask/vX.Y.Z` cuts that tag the same way `make tag TAG=vX.Y.Z` cuts the tool's, and a tool tag is refused while `mask/` (Markdown aside) differs from the mask tag `go.mod` requires; goreleaser builds with `GOWORK=off` so the release compiles that same version (T-0290).
...
What broke, and what changed (T-0155). **v0.0.1, 2026-09-22:** the release job failed at "Set up job", before its first step, because `sigstore/cosign-installer@v4` does not resolve: that action publishes exact `v4.x.y` tags and floating `v3`/`v2`, but no floating `v4`. Nothing was published — no GitHub release, no cask — and the tag stays as the record. The workflow now pins `@v4.1.2`, and every `uses:` in `release.yml` was checked against the action's real tags with `gh api repos/<owner>/<repo>/git/ref/tags/<ref>`; do that for any new action, because no push to main runs this job and a tag is the only thing that exercises it. Each failed proof is followed by the next `v0.0.x`, never by moving the tag: the tag is immutable even when it published nothing. **v0.0.2, 2026-09-22:** every check passed and goreleaser built all six archives and their SBOMs, then cosign failed to sign `checksums.txt`: the installer's default is cosign v3, whose `sign-blob` writes one Sigstore bundle and ignores the v2-era `--output-signature`/`--output-certificate` flags, so it tried to open an empty bundle path. `.goreleaser.yaml` now asks for the bundle (`--bundle=${signature}`, written as `checksums.txt.sigstore.json`, goreleaser's own cosign-v3 shape), so a release carries one signature file instead of a `.sig`/`.pem` pair. Verify a download with `cosign verify-blob --bundle checksums.txt.sigstore.json --certificate-identity-regexp '^https://github.com/Liarea/lazyslice/' --certificate-oidc-issuer https://token.actions.githubusercontent.com checksums.txt`, then check the archive's line in `checksums.txt`. Nothing local exercises signing (`make snapshot` skips it), so this too is proven only by a tag. `make tag` (`tools/release/tag.sh`) now runs every precondition above before a tag is cut. **A mask tag (`mask/vX.Y.Z`, cut with `make tag TAG=mask/vX.Y.Z`) is not a version of the tool**: goreleaser ignores the pattern (`git.ignore_tags` in `.goreleaser.yaml`), because the first one, on a later commit than v0.1.0, made goreleaser read `mask/v0.1.0` as the tool's version and fail every snapshot with "invalid semantic version" (CI's release-config job, 2026-09-22). **v0.0.3, 2026-09-22: the pipeline is proven.** The release run published six archives, six SBOMs, `checksums.txt` and its Sigstore bundle, and pushed the cask to the tap; `brew install Liarea/tap/lazyslice` on a Mac that had not built the release installed the CI-built binary, which printed `lazyslice 0.0.3` with its commit and build time and was not stopped by Gatekeeper; `cosign verify-blob` with the identity regexp and issuer above answered `Verified OK`, and the downloaded archive's SHA-256 matched its line in the signed file. Two things to know: Homebrew warns that the cask's `postflight` hook is deprecated in favour of `postflight_steps` (T-0277) — Homebrew 7.0.0 (2026-09-13, https://brew.sh/2026/09/13/homebrew-7.0.0/, "Install steps") moved `preflight`/`postflight`/`uninstall_preflight`/`uninstall_postflight` to the declarative `*_steps` stanzas; official taps already reject the old ones, and a third-party tap like `Liarea/homebrew-tap` gets the warning above until **2027-12-11**, after which the old stanza is rejected too. That is the day it bites: every fresh `brew install` on macOS would then stop at the cask failing to load, before Gatekeeper even gets a say — no binary staged, no quarantine attribute to remove, nothing installed. It cannot be fixed early: as of 2026-09-22, goreleaser (v2.18.2, https://goreleaser.com/customization/homebrew_casks/) can only render the deprecated `hooks.post.install` block; the PR that would add a `hooks.post.install_steps` to render `postflight_steps` instead (https://github.com/goreleaser/goreleaser/pull/6873) is open and unmerged. Revisit `.goreleaser.yaml`'s `homebrew_casks.hooks` once that PR ships in a release, well before the December 2027 date — CI's release-config job (`.github/workflows/ci.yml`) fails on its own if a future goreleaser upgrade starts emitting `postflight_steps` without this being updated to match. And cosign is not one of `make tools`' binaries, so verifying locally means `GOBIN=$PWD/bin/tools go install github.com/sigstore/cosign/v3/cmd/cosign@v3.1.1` first. Gate 3's "`brew install` from your tap installs a binary that prints its version" is met three tags after it was written.
