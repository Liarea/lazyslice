...
| `verify.refused.second_net_document` | verify | 9 | {table}.{column} is not masked and {count} of its values validate as {reason}: no category but semi_structured accepts a json, jsonb or hstore column, so mask it with --mask {table}.{column}=semi_structured, or leave its table out with --skip-table {table} |
