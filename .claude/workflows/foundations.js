...
const tasks = [
  { id: 'T-FIXTURES', title: 'Golden fixtures: Pagila and nasty.sql', model: 'opus', stage: 'foundations', paths: ['testdata/', 'internal/testutil/fixtures.go'],
    brief: `Build ${REPO}/testdata/ with two PostgreSQL fixtures. (1) Pagila: download the schema and data SQL from github.com/devrimgunduz/pagila (pin the commit in testdata/README.md) into testdata/pagila/. (2) testdata/nasty.sql, a hostile schema you design, containing: a self-referencing table; a three-table foreign-key cycle; a composite primary key with a composite foreign key to it; a polymorphic association (owner_type, owner_id) with no constraint; a JSONB column containing emails and phone numbers nested two levels deep; a free-text notes column containing full names; a boolean column named email_verified (false-positive trap); a column named ref holding email addresses (false-negative trap); a partitioned table with two partitions; an enum type; a text[] array column with emails; a generated column; identity columns with non-default sequence starts; a table in a schema other than public; a quoted mixed-case identifier; and a function that generates 2,000,000 rows into a table for streaming tests, gated behind a psql variable so normal loads stay fast. Add internal/testutil/fixtures.go with LoadPagila(ctx, url) and LoadNasty(ctx, url, big bool) using the existing testutil container helper. Document every trap in testdata/README.md with the behaviour lazyslice must show for it. Add a unit test that loads both fixtures into a container and asserts table counts.` },
  { id: 'T-INVARIANTS', title: 'Invariant test suite I1 to I6', model: 'opus', stage: 'foundations', paths: ['internal/invariants/'],
    brief: `Write ${REPO}/internal/invariants/, a black-box integration suite (build tag integration) that runs the built lazyslice binary against two PostgreSQL containers from internal/testutil and both fixtures, and asserts the six invariants in ${REPO}/ARCHITECTURE.md: I1 every foreign key in the target resolves (query information_schema and check each FK with an anti-join); I2 no flagged column in the target contains a value the classifier's own detectors would flag (call the classify command with --json on the target and assert zero hits, and additionally grep for every email and phone that existed in the source); I3 same source, same secret, same config yields a byte-identical target (pg_dump --data-only both targets and compare); I4 the source is unchanged (row counts and a per-table md5 of ordered rows before and after); I5 the emitted lazyslice.yml, fed back in, reproduces the snapshot (compare with I3's method); I6 the root table row count equals --take. Structure it so each invariant is one test function that fails with a message naming the table and column. Since the pipeline is a no-op today, every test must currently FAIL for the right reason, not skip; add a helper that skips only when Docker is unavailable. Wire make integration to run this package.` },
  { id: 'T-CLAUDEMD', title: 'Per-directory CLAUDE.md files', model: 'sonnet', reviewers: 1, stage: 'foundations', paths: ['cmd/CLAUDE.md', 'internal/CLAUDE.md', 'internal/*/CLAUDE.md', 'testdata/CLAUDE.md', 'docs/CLAUDE.md', 'tracker/CLAUDE.md', '.claude/CLAUDE.md'],
  ...
  { id: 'T-ADR008', title: 'ADR-008 first run, from lazygit, lazydocker, k9s source', model: 'opus', reviewers: 1, stage: 'foundations', paths: ['docs/adr/008-first-run.md', 'research/FIRST_RUN_STUDY.md'],
    brief: `research/OPEN_QUESTIONS.md item 5 forbids writing the first-run ADR from sqlit alone. Clone lazygit, lazydocker, and k9s into /private/tmp/claude-501/lazyslice-scratch/ and read the code paths that run on first launch with no config: what they detect, what they ask, how they discover Docker contexts and kube contexts, how they degrade when nothing is found, and how they surface keybindings. Write research/FIRST_RUN_STUDY.md with file-and-line citations. Then write docs/adr/008-first-run.md (Status: proposed) specifying lazyslice's first run exactly: the source-target-root question ladder, the single blocking question and its default, the target eligibility gate as ARCHITECTURE.md defines it, Docker context resolution order (DOCKER_HOST, current context in ~/.docker/config.json and its meta, default socket paths), compose and .env as naming sources only, and every question the tool may ask with its default. It must not contradict ARCHITECTURE.md or ADR-004; where they are silent, decide and say so.` },
...
].map((q, i) => () => agent(`You are a reviewer on lazyslice. Repo: ${REPO}. Read ${REPO}/CLAUDE.md and ${REPO}/ARCHITECTURE.md. ${q} Return findings; high severity means the phase 3 gate is not met.`,
