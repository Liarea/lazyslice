...
Release notes between two versions are generated from commit bodies (`make relnotes FROM=v0.1.0 TO=v0.2.0`, `tools/relnotes/relnotes.py`), so every commit that changes behaviour carries bullets a user can read. Gareth, 2026-09-16: one-line commits that only reference tickets are unusable for that.
