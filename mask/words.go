...
//
// The person-name lists are not here: givenNames and surnames below are built
// from censusGivenWords and censusSurnameWords (words_corpus.go, generated by
// tools/names from the U.S. Census Bureau's 2020 Census name files, public
// domain; THIRD_PARTY_NOTICES.md). They are the module's one pair of name
// lists (T-0304, ADR-015): personNameMasker's three roles and emailMasker's
// local part all draw from them, and vocab.go's Emits answers over them. They
// are real, common names on purpose. A masked first_name equal to some other
// customer's real first_name is the ordinary case on any real table, and the
// residual scan explains it by count and row identity rather than refusing it
// (ADR-015); what it may never do is equal its own row's source value, and
// maskCell's redraw is what guarantees that.
const (
...
// role -- the given-name list for RoleGiven, the surname list for RoleFamily
// -- in the order Mask indexes them, or nil for RoleFull, which draws a pair
// rather than a single list. It is not part of the masking algorithm section
// 5 fixes; nothing here reads it.
//
// It is on its way out, and new code should not call it. Since T-0304 the
// role lists are the module's one pair of name lists (the 2020 Census given
// names and surnames), so there is no separate role vocabulary left to audit,
// and T-0304 was asked to remove it. It stays only because the parent
// module's internal/verify tests still call it and this module lands first,
// on its own; it is removed in the next mask minor once they no longer do
// (tracker T-0337). It carries no formal deprecation marker for the same
// reason: the parent's lint would fail on those callers.
