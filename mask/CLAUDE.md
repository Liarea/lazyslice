...
## `words_corpus.go` is not yet read by any masker (T-0303)

`words_corpus.go` (`// Code generated by tools/names; DO NOT EDIT.`) carries
two more word lists, `censusGivenWords` and `censusSurnameWords` — 958 given
names and 1,000 surnames from the U.S. Census Bureau's 2020 Census top-1000
files, regenerated from `tools/names/*.csv` by `go run ./tools/names`
(`tools/names/README.md`, `THIRD_PARTY_NOTICES.md`). **No generator in this
module reads either constant yet.** T-0303's own paths stop at generating
and committing the file; wiring it into `personNameMasker` — most plausibly
as a wider `RoleGiven`/`RoleFamily` source, since T-0292 (`words.go`'s own
"do not read `givenNames`/`surnames`" note, above) already names "a
materially larger admissible domain" as the actual fix `roleGivenWords`/
`roleFamilyWords`' synthetic-syllable corpus was a mitigation for rather
than a substitute — needs the review that decides where it plugs in, the
`newWordList`/`Domain()` wiring, and a determinism-scope release note of its
own (this file's "Determinism scope" bullet, above: changing what a category
reads changes every value that category already masks). That is tracker
T-0304, scoped to land after T-0302. Until then `words_corpus.go` is
committed, generated, tested (`mask/words_corpus_test.go` pins its counts,
