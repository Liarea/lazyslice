...
**Status 2026-09-25 (before the v0.4.0 tag):** the dogfood step is landed in full (docs/DOGFOOD_LOG.md; T-0311 to T-0361 and their follow-ups), with `--mask` (T-0319, T-0364), both first-run questions at a terminal (ADR-017, T-0343), the committed container target (ADR-016, T-0327), free text fitted to the input's length (mask/v0.5.0, T-0328) and json documents masked leaf by leaf (T-0272). That last decision masks less on purpose, so it got the red-team round THREAT_MODEL T1 asked for before it ships: docs/reviews/2026-09-25-redteam-json/ round 1 (37 attempts, 27 leaks, 16 named residuals, 11 undocumented) became the eight jsonfix tasks (T-0393, T-0394, T-0397, T-0398, T-0399, T-0402, T-0403, T-0404), and round 2 replayed the eleven against 5a77f1b: ten hold, one is the named residual (a national-format phone number used as a document key with no `--phone-region`, T-0401), zero undocumented, which is the stopping rule gate 5 set. v0.4.0 is cut after the docs truth pass (T-0407) lands.

**Status 2026-09-24:** v0.3.0 is released (cut by `make tag` after green CI). What it carries: masked names are real common names from the 2020 Census lists (ADR-015, proposed: the residual scan explains a coincidence with another row's real name by the masker's own vocabulary, a count of what it emitted and a same-row check, and still refuses every passthrough; T-0302, T-0303, T-0304), `go install` of the tag works, and the three dogfood sessions are in docs/DOGFOOD_LOG.md with their findings filed as T-0311 to T-0334. Gate 6 still needs the posting, which the maintainer decided (2026-09-24) goes out with v0.4.0 rather than now, and the dogfood found the copy of a Rails application does not boot yet (state columns swept into masking, framework tables empty, configuration JSON masked leaf by leaf, one refusal per run), which the E6 tasks from T-0311 on carry; v0.4.0 is the release those land in.
