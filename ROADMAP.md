...
- [ ] (unticked 2026-09-09: `make torture` exits 1 until T-0113 re-cuts regressions 004 and 007, which still assert the refusal the credential fix removed) Nine of ten torture schemas snapshot cleanly and the tenth fails naming its cause (docs/TORTURE.md, T-TORTURE, 2026-09-08). The ten runs demand twenty-seven flags between them: nineteen `--unmask`, seven `--skip-table`, one `--key`. It was forty-five and thirty-seven `--unmask` at T-TORTURE; eighteen of those were one defect, unique credential columns (T-0098), fixed in the harden step by `mask/gen_credential.go` and re-measured by T-0112 (2026-09-08). Two of the eight reductions in `testdata/regressions/` still assert the exit-12 refusal that fix removed, so `make torture` itself does not yet exit 0 — T-0113.
