...
type Classification struct {
    ...
    Fingerprint string      // sha256 over (rule-pack version, and per column: category, masker)[:16]; §5 "Determinism scope"
}
...
plan(schema, cls, req, priv):
    ...
    for t in tables:
        identity[t] := req.Keys[t] ?? PK(t) ?? nonPartialNonExprUnique(t) ?? probedPseudoKey(t, r)
                       // an explicit key always wins over a probed guess (§3.4)
        if identity[t] == nil: refuse(t, exit 12, "--key t=col,col")
...
- **NULL stays NULL, `''` stays `''`.** Credentials become a fixed unusable value (`$lazyslice$invalid`). Free text and JSON are replaced whole (§4).
...
| `--docker-host` | resolved (`DOCKER_HOST`, context, default sockets) | discover | Docker endpoint for container discovery |
...
| Q1′ | `target <name> is stopped — start it? [Y/n]` | Yes | eligible target's container is exited | takes the default | `--target` |
...
**The repository** (`internal/repo`; THREAT_MODEL.md T6). When lazyslice creates `./lazyslice.secret`, or first reads a yml naming a `mapping_file:`, it protects the file before writing or using it, in this order, and prints what it did:
