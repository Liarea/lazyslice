...
**"Strong" is email, phone and payment card** — the three value shapes a parser decides rather than a dictionary guesses (`internal/textsig`). The narrowing is deliberate and is stated in THREAT_MODEL.md T1: this text is SQL, so a `CHECK` is full of English words and a default is full of identifiers, and the dictionary-backed signals of §4 over them would refuse ordinary schemas. A name or a street address in a `CHECK` on an unmasked column is therefore not found, which is the same class of miss T1 already admits.

**§6 gains a catalog pass for the same boundary, from the other end.** After the load, verify reads the target's own `pg_attrdef` (which holds both the defaults and the generated expressions), `pg_constraint` (the table `CHECK`s and exclusions, and a domain's `CHECK` with them) and `pg_index` (a partial index's predicate and an expression index's key expressions, which have no `pg_constraint` row at all), runs the same three validators over every literal, and fails **exit 9** (`verify.refused.catalog_literal`) on a hit. It is not the same code path as the plan's check — the planner reads the source's `*Schema` and this reads the target's catalog — so a literal that reached the target by a route the planner does not walk is still found. `internal/plan/ddlliteral.go` and `internal/verify/catalog.go` are the two halves; `testdata/regressions/011-masked-column-default-holds-a-literal.sql` is the guard.
...
Two bounds are recorded rather than papered over. The **plan-time** pass reads no index predicate and no domain `CHECK`, although §11.1 recreates both (tracker T-0163): the catalog pass finds both, so such a literal is exit 9 after the target is loaded rather than exit 12 or 13 before anything is dropped. And `internal/core` does not yet fill `pipeline.PlanRequest.Key` (tracker **T-0161**), so arm 1 is implemented and dormant: until that wiring lands, a masked default whose literal a strong validator hits is refused at exit 13 under arm 2's last clause rather than masked, and `testdata/regressions/011`'s header says `exit 13` rather than `ok`. That is the fail-closed direction and it is not the finished behaviour. The catalog pass is unaffected by the gap because of the `DefaultOriginal` keying above: with no default rewritten, no default is exempt, and the masked-default class is judged like every other object rather than waved through by an exemption written for a rewrite that has not happened yet.
