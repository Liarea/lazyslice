...
One bound is recorded rather than papered over. The **plan-time** pass reads no index predicate and no domain `CHECK`, although §11.1 recreates both (tracker T-0163): the catalog pass finds both, so such a literal is exit 9 after the target is loaded rather than exit 12 or 13 before anything is dropped.
