...
// LeafMap is LeafKeys when Category is semi_structured, Source is ByClassifier and LeafNameCategory is empty, nil otherwise (every leaf masked; §4's T-0272 and T-0393 amendments).
...
type Mask struct {
    Category Category // free_text for the bare --mask TABLE.COL
    By       string
}
...
**Type additions recorded after implementation (2026-09-23, T-0302, ADR-015, proposed):** `Residual` gains `AddEmitted(col ColumnRef, path string, canonical []byte)` and `Emitted(col ColumnRef, path string, canonical []byte) int64`. For every masked scalar cell and array element of a column whose masker has a vocabulary (`mask.Emitting`; only `person_name` today, never a JSON leaf), transform counts one against the first 64 bits of `HMAC(runKey, enc("emitted", column, path, canonical(output)))` in a mutex-guarded map that stores no value; verify reads the count back for a residual hit inside that vocabulary (§6 item 3). The plan counts 16 bytes per distinct output, bounded by the vocabulary size, against `--memory-budget` beside the filter. `mask` gains `Emits(id, value, constraints) bool`, its value-free half `Emitting(id, constraints) bool`, and `FoldEqual(a, b string) bool`, the letters-and-digits comparison its post-condition fires on.
...
**3.8 Framework metadata tables (added 2026-09-24, T-0314).** A fixed, named set of migration-bookkeeping tables — `schema_migrations`, `ar_internal_metadata` (Rails); `flyway_schema_history` (Flyway); `django_migrations` (Django); `alembic_version` (SQLAlchemy/Alembic); `knex_migrations` (Knex.js); `goose_db_version` (Goose); `__EFMigrationsHistory` (EF Core); `atlas_schema_revisions` (Atlas); `_prisma_migrations` (Prisma); `SequelizeMeta` (Sequelize); `DATABASECHANGELOG` and `DATABASECHANGELOGLOCK` (Liquibase) — is copied whole as a `Lookup` step regardless of §3's ordinary lookup rule: no outgoing-edge, incoming-edge or row-ceiling test applies, because such a table's ordinary shape is exactly what the rule would otherwise exclude it for (no foreign key reaches it at all) or exempt it despite (a small, fixed row set an ORM reads before the application does anything else). The match is on the bare table name alone, case-insensitively, never the schema. §4's masking threshold does not apply to a column of one that is also on that table's own fixed list of the tool's known bookkeeping columns (a version string, a checksum, a timestamp, a boolean flag) — every such value is bookkeeping the tool itself wrote and reads back, never end-user data, so "when in doubt, mask it" (root CLAUDE.md) has no doubt to resolve for it. A column of a recognised table that is *not* on its own bookkeeping list — Liquibase's `AUTHOR` (the developer who ran the changeset), Flyway's `installed_by` (the database role or OS user who applied it) — is not covered by this exemption at all and reaches §4's ordinary classification exactly as if its table were not on the fixed set (T-0314 review round, finding 2): the table is still copied whole as a `Lookup` step regardless of what any one column decides. Rails' `ar_internal_metadata` gets one further rewrite: the row with key `environment` has its value set to `development` in the target, inside that table's own load transaction, so a snapshot does not carry the source's own environment into a checkout where it would make `bin/rails` refuse a destructive task. The rewrite is reported on the table's own plan line before any row moves. Dogfood session 1 found both halves of the failure this closes: `schema_migrations` reached by no foreign key planned `SchemaOnly` (zero migration rows, so a fresh checkout re-runs every migration), and `ar_internal_metadata` copied verbatim carried `environment: production` into a development database.

**3.9 Refusals are collected, not stopped at the first (added 2026-09-24, T-0318).** §3.4's identity ladder, §3.6's cannot-skip-a-parent case, the write-back check of §4 and the unique-index domain rule of §5 each ends a run at exit 12; the plan now finds every one of those it can in a single pass rather than returning at the first. A table with no row identity is dropped to `SchemaOnly` — the same way an unreadable child-only table already is — so the walk and the unique-index check still run over the rest of the schema and can report their own findings alongside it. The process still exits 12 exactly once: every one of the four causes shares that exit code, so the run's own exit status and the one refusal `cmd/lazyslice` names after the transcript are the first cause found, in the order the checks above run, while the transcript itself prints one line per cause. This is what dogfood session 1's own second run needed and did not have: it took nine runs to reach a green `verify`, refused on the next single cause each time — one join table with no identity, its identical twin only named once the first was cleared, then ten masked columns under unique indexes named one per run. A table of three columns or fewer that reaches the identity refusal now has its own columns named in the `--key` hint instead of the generic `--key TABLE=col,col`: a Rails join table's composite key is exactly its own columns, and the placeholder made an operator open the schema to answer a question the refusal already knew.
...
- **Free text and JSON.** `text`/`varchar` columns named like `notes|comment|description|bio|message|body|reason` or with dictionary hits in samples are `free_text` and replaced whole with filler whose length is derived from `h` inside the column's admissible range (§5); the source length never survives. **Amended 2026-09-24 (T-0328, the maintainer's decision in T-0323):** the filler is now fitted to the input's length, so the approximate length survives (§5's T-0328 amendment). `json`/`jsonb`/`hstore` documents are masked whole, as CONCEPT.md promises: structure and key names are kept and **every scalar leaf is replaced**. String leaves go through the category masker chosen by running the leaf's key name through the name rules, defaulting to `free_text` for a key no rule recognises; number leaves become a number of the same kind (integral or fractional) derived from `h`; boolean leaves become a boolean derived from `h`; `null` stays `null`; arrays and objects keep their shape. Wildly varying keys, or any `json`/`jsonb` in a table the rule pack's single `log_shaped` rule matches (`rules.yml`'s `tables.log_shaped`: an underscore-bounded `audit(s)`/`log(s)`/`history`/`histories`/`event(s)`/`activity(ies)`/`trace(s)` word over the table's normalised — case-split, lower-cased — name, so a CamelCase `AuditLog` matches too), replace the document with `{}`. Key names are the one thing a document keeps, and §6 item 6 says so. `bytea` in a person-shaped column is `binary_personal` and set to NULL. **Amended 2026-09-25 (T-0272, the maintainer's decision in T-0143 of 2026-09-24):** a `json`/`jsonb` document is masked *per leaf*, not every leaf. The classifier collects every object key in the column's samples and records each on the decision (`Decision.LeafKeys`) with the category the name rules give it, or `none`; transform and verify read that map. A leaf whose nearest enclosing key with a category names one is masked by that category's masker; a leaf whose value one of the value validators recognises (the classifier's leaf questions plus every validator the second net runs over a leaf) is masked by the validator's category; a leaf whose every enclosing key the samples showed, all `none`, and whose value no validator recognises is **copied**; every other leaf — a key the samples never showed, or a document with no key at all — is masked as `free_text`, as before. A category keeps its own masker for a leaf only where that masker's output cannot coincide with another row's real value at the same path (email, phone, address, geo, national_id, financial_account, network_id, online_id, credential); `person_name`, `person_date` and `special_category` leaves are masked as `free_text`, because a leaf residual hit is never explained or probed. Number and boolean leaves follow the same copy-or-mask rule and are re-derived from `h` as before when masked. Keys stay. The log-shaped-table collapse to `{}` and the `hstore` collapse are unchanged. A decision with no map (no samples) masks every leaf, as before. **The column's own decision is the root of every leaf's chain** (the T-0272 review round): the map is read only when the column's decision is the classifier's plain `semi_structured` verdict (`Decision.LeafMap`). A document column whose own name the rule pack scores with a personal category — a `jsonb` `medical_history` is `special_category` on its name alone — and one an operator raised (a yml pattern or column entry, a committed `mask:` block, `--mask`) have every leaf masked as `free_text`, as before this amendment; raising a document column is how an operator puts whole-document masking back on a column that holds personal data under keys no rule knows. The value half reads a phone number under the run's `--phone-region` (or the yml's `phone_region`) as well as in international form, the same question the second net asks, so a national-format number under a key no rule names is masked, not copied and then refused at exit 9. What this gives up is stated in §6 item 6 and THREAT_MODEL.md T1: a personal value that no name rule on its keys and no validator on the value recognises, inside a plain `semi_structured` document, is now copied. **Amended 2026-09-25 (T-0393, the JSON red team's round 1):** the review round's "column's own decision" rule did not reach a document column whose own name matched a personal rule that does not accept `json`/`jsonb` — every rule but `special_category`'s — because the type decided such a column plain `semi_structured` and its decision looked like one nothing names; a `jsonb` `full_name`, `passwords` or `notes` copied every signal-free leaf. The rejected name's category is now on the decision (`Decision.NameHit`), `LeafMap` is nil for it, and every leaf is masked under that category (`Decision.LeafNameCategory`) through the same leaf-masker choice a per-leaf category gets: the category's own masker for `email`, `phone`, `address`, `geo`, `national_id`, `financial_account`, `network_id`, `online_id` and `credential`, `free_text` for every other. The leaf's value is not consulted. A `special_category` name and an operator's raise keep `free_text` for every leaf. **Amended 2026-09-25 (T-0398, the same red team's entry 26):** the log-shaped match above is the rule pack's one regex, read once by `internal/classify` and carried on every column of the matching table as `Decision.LogShaped`, whatever the column's own family. `internal/transform`'s `maskDocument` and `internal/verify`'s own restatement of the per-leaf rule (`jsonleaf.go`) read that field now, in place of a second copy each package used to keep: `internal/transform`'s own copy matched a fixed eight-word list, split the table name on `_` alone with no CamelCase normalisation, and carried neither `activity` nor `trace` — so a CamelCase table (Prisma's default `AuditLog`) or an `*_activity`/`*_trace` table was reported "replaced whole" by the reasons line above and walked leaf by leaf instead, copying every signal-free leaf. Before T-0272 that mismatch cost nothing, because every walked leaf was masked regardless; T-0272's per-leaf rule is what turned it into a leak.
- **Never masked, always explained:** generated columns (never copied; the target recomputes them), surrogate keys (`id bigint` and the FK columns that reference them), and a §3.8 framework metadata table's own bookkeeping columns (T-0314; not every column of the table — see §3.8's own qualification, T-0314 review round finding 2). Surrogate keys are preserved verbatim, which means every snapshot carries an exact join key back to the production row; THREAT_MODEL.md "Positions" and §6 item 6 state that first. There is no copy-as-is exemption by type: an enum column, a partition-key column and a `varchar(2)` column are classified like any other, and a masked enum emits a valid label (§5). The earlier draft's enum exemption was a copy-as-is default by type and is removed.
...
**Recorded after transform landed (2026-09-06, T-EXTRACT):** every JSON string leaf is masked as `free_text`, §4's own default for an unrecognised key, rather than through the name rules; the shape of the fake is lost (an email leaf becomes filler) but coverage is unchanged, and the residual entry for every leaf is derivable by verify. §14's one-level JSON key collection in classify is what restores per-leaf categories, by putting them on the Decision where verify can read them. **Superseded 2026-09-25 by T-0272** (§4's amendment above): the categories are on the Decision now, and the residual entry for a masked string leaf is still `free_text`'s canonical form whichever masker replaced it, so verify derives it exactly as before.

**Amended 2026-09-16 (T-0221): the phone signal reads a region, configured or guessed.** The "libphonenumber `IsValidNumber` with region hint" wording above was true of the code for exactly one hint, `internal/textsig.PhoneRegionHint` ("ZZ", the unknown region), which admits only a number already written in international form. A national-format number — `07911 123456`, `020 7946 0958` — scored zero however many rows agreed, whatever the column was named, because nothing offered the parser a region to read it under; the 2026-09-15 round-3 red team's `kontaktnr`/`contact` finding (`docs/reviews/2026-09-15-redteam/round3-still-leaking.json`) is a plain domestic phone number, no obfuscation, in a column name no pattern in `rules.yml` matches, crossing into the target verbatim under exit 0.

`--phone-region REGION` (and the yml's own `classify.phone_region`, which carries it forward so a re-run needs no flag) names the region a candidate is additionally parsed under, on the same `strong` footing the international-only entry already has: the row path decides the column outright on this evidence, exactly as it does for a `+`-prefixed number, and the reason line says `phone region assumed: <REGION>` so the operator can see the assumption instead of inferring it from a flag they may not have typed on this run. `internal/verify`'s second net asks the identical question over the loaded target — the configured region and the international form, and nothing else — so the two nets never disagree about what "phone" means on a given run.

**Alongside whatever region is configured, a short, fixed list of common regions is also tried, and a hit decides nothing on its own.** Trying every candidate region unconditionally would mask a column the moment some region's numbering plan happened to fit — measurably, for the same reason a bare national-identifier checksum clears a meaningful fraction of a random digit run of the right length (§4's national_id note, `internal/verify/validators.go`'s own comment) — so a ten-digit account number or an order code would cost a real column to a guess nobody asked the tool to make. A guessed-region hit is therefore masked only with **corroboration**: a proven personal column (`likely` or `certain`) elsewhere in the same table — the identical signal `internal/verify`'s national_id digits entry already reads off `Decision.TableHasLikelyPersonalColumn` for its own corroboration gate (T-0187's third review round). Without it, the column is left exactly as it was: unmasked, and the reason line says so rather than reading as a clean bill of health for a column nobody looked inside. (A column whose own name already matches `rules.yml`'s phone pattern needs no corroboration from this pass: `decide`'s ordinary name-match branch already masks it, at `possible` or above, before `guessedPhoneColumns` ever sees it, because that pass only looks at a column still below `possible`. A first landing read the name match a second time as a corroboration arm here too, but that arm could never fire — a T-0221 review round proved it dead and removed it rather than leave a named gate no input could reach.) This pass runs whatever `--phone-region` holds (a review round found the first landing skipped it entirely once a region was configured, which meant naming a region *replaced* the fifteen-region fallback rather than adding to it — a real cost for the common case this flag exists for, a multi-country database, where the operator's own region is one more region to trust and never a reason to stop looking at the others); a configured region simply becomes the one region trusted without corroboration.

This is a recall change against THREAT_MODEL.md T1 and is reviewed as one; THREAT_MODEL.md's own amendment records the corroboration trade in T1's terms.

**Amended 2026-09-24 (T-0317, dogfood session 1): a version, build or release column is not a network address, and a key, code, license, serial or token column is not a phone number.** Dogfood session 1's `last_player_version` held dot-separated integers of the same shape an IPv4 address is (`1.2.3.4`); 93 of 168 samples parsed as one, below the 80% threshold, so the **Signals** bullet's strong-hit rule (§4, T-0136: "a strong validator matching at least one sample below the category threshold masks the column as `free_text`") masked the whole column on the coincidence. For a column whose normalised name carries `version`, `build` or `release` as a whole word, wherever in the name it sits, the classifier no longer offers the `network_id` validators (IP, MAC) as a *minority-hit* signal — the name check runs regardless of the values, since a version string's shape is a property of the column's purpose and not of any one sample, but a majority of the column's own values genuinely parsing as addresses still masks it `network_id` as before (a review round on this task found the first landing withheld that case too, which is not what this amendment is for). Separately, `license_key` held ten-digit samples, 8 of which cleared one of the guessed-region phone corroboration's fifteen numbering plans (the guessed-region amendment above), and the same corroboration masked it `phone` beside a personal neighbour — the wrong shape for a key. The guessed-region fallback (only — a column whose own name already matches the phone name rule is unaffected, as the amendment above already states for that branch) is no longer offered a column whose normalised name carries `key`, `code`, `license`, `serial` or `token` as a whole word. Neither veto removes any other signal: a version column with a genuine email or phone value is still decided by that, and a key or code column beside a `certain` personal neighbour with no other signal is still swept into `free_text` by the neighbouring-column rule's second arm (T-0311, above) rather than left unmasked — only the specific, wrong category is withheld. `TestPagilaPrecisionAndRecall` and `TestFiftyNamesFromThreeSchemas` are unchanged by this diff (precision 0.762/0.959, recall 1.000/1.000, neither truth set carrying either shape), which is this change's own T1 measurement: nothing personal in either hand-labelled set is masked less than before. **Owed:** THREAT_MODEL.md T1 is outside this task's paths and does not yet carry this amendment — tracker T-0359 carries the edit. `internal/verify`'s second net has no matching veto for the `network_id` case, unlike T-0315 and T-0316's mirrored ones — tracker T-0360 carries that.

**Amended 2026-09-24 (T-0316, dogfood session 1): the card signal wants an issuer prefix, and under an identifier's name the issuer's length.** The **Signals** bullet's "Luhn" masked eight identifier columns of a production Rails schema — customer, subscription, invoice and estimate numbers and `schema_migrations.version` — as `free_text` on a minority of values that passed the check digit by chance, one digit run in ten. The classifier's card entry is now `textsig.ValidCard`: twelve to nineteen digits, the Luhn check digit, and a prefix inside a known issuer identification range (`internal/textsig/card.go` carries the table and its source). A column whose normalised name ends in `id`, `number`, `num`, `no`, `nr`, `version`, `ref` or `reference` is scored with `textsig.CardShape`, which also requires a length the matching issuer issues. The category, the phrase ("pass the Luhn check"), the entry's strength and its place in the order are unchanged. `internal/verify`'s two card entries (character, strong; numeric, ratio) are each split by the same name rule, so the second net asks exactly what the classifier asked. `textsig.ValidLuhn` stays as it was for the DDL-literal passes and the JSON key and leaf readers. THREAT_MODEL.md T1's T-0316 amendment has the measurement and the residual.

**Amended 2026-09-24 (T-0315, dogfood session 1): what the Shannon-entropy signal does not read as a secret.** The **Signals** bullet's "Shannon entropy for secrets after the UUID check" masked, on a production Rails schema, six file-name columns, four MD5 and two file-fingerprint columns, three single-table-inheritance `type` columns, a formatter, a component name, an environment variable's name and two columns holding one and four values as `credential`, "N/N samples look like secrets", to the fixed `$lazyslice$invalid` — and a class name that literal replaced raises on every row the application loads. Five rules now bound it, and the real secrets the same run caught (API keys, tokens, password hashes, OAuth secrets) are still caught. (1) `textsig.LooksSecret` refuses four value shapes before it measures entropy, as it already refused a UUID and a URL: a name ending in a known file extension whose stem carries no word from the name dictionary; a hex digest of exactly 32, 40 or 64 characters in one case, bare, under an `md5`/`sha1`/`sha256` prefix, or as 16, 20 or 32 colon-separated byte pairs; two or more identifier words joined by `::` or `.` (a Ruby constant path, a dotted class name), where each word must read as one a person wrote, so a JWT's or a bot token's random segments are not one, and a `.`-joined one is spared only when a segment is camelCase and no word is in the name dictionary, so a dotted handle (`katherine.johnson84`) is still read; and an upper-case environment variable's name (`AWS_S3_BUCKET_V2`) whose words carry at most two digits each and no dictionary word. A value with a file extension answers to the file-name rule alone, so a name-bearing file name joined by underscores is not spared as a dotted identifier. (2) The classifier does not ask the entropy validator about a column named `type`, `klass` or `component_name` (after name normalisation), because an un-namespaced class name clears the entropy floor with nothing in the value to tell it from a token; every other validator and every name rule still runs. (3) The entropy validator decides a column only over five non-NULL samples or more; below that it can record `low` like any weak signal, and a column whose name says credential is masked on the name as before. (4) A column in which at least a fifth of the samples are file names whose stem carries a dictionary word — the share T-0313's bare-name rule reads as a column of people — counts every file name as the entropy validator's hit, so a column of documents named after their owners is masked whole ("N/N samples are file names, a fifth or more carrying a dictionary name") and not copied with the rows whose names the dictionary lacks. (5) The rule pack's credential pattern gains `key_?hash(es)?`, because a hash of an API key is a 64-character hex digest and rule (1) no longer reads its value. `internal/verify`'s second net applies (1), (3), (4) and the three names of (2) to its own credential entry, so it does not refuse at exit 9 a column the classifier left unmasked on purpose. THREAT_MODEL.md T1's T-0315 amendment states what this gives up: a secret of exactly a digest's hex length, a secret column of fewer than five rows, and a secret in a column named `type`, `klass` or `component_name`, each under a name no credential rule matches.

**Amended 2026-09-24 (T-0313, dogfood session 1): the bare word `name` needs corroboration.** The **Scoring** bullet's "name hit alone → `possible`" masked, on a production Rails schema, 38 columns called just `name` — tags, folders, playlists, widgets, roles, languages, AI models, triggers — and every Paperclip `*_file_name` column as `person_name`, and two of them, under unique indexes, refused the plan. The word says a column holds *a* name, not *a person's*. The rule pack's `person_name` row no longer carries it (it carries instead the qualifiers that say the name is a person's — `legal_name`, `preferred_name`, `nick_name`, `cardholder_name`, `billing_name`, `shipping_name`, `name_on_card`, `name_given` and their kin — which keep `possible` on the name alone); a separate `bare_name` row (`name`, `display_name`, and so every `<thing>_name` no more specific rule claims) reaches `possible` only with one of three things behind it: (1) a word for people in the table name or in the column name — `users`, `customers`, `clients`, `staff`, `members`, `authors`, `accounts`, `profiles` and about seventy more, singular or plural, listed in `rules.yml`'s `corroborated_by` — so `users.name` and `orders.customer_name` need no sample; (2) fewer than three non-NULL samples, where the name decides alone exactly as before, because an unproven column is not a clean one; (3) at least a fifth of the samples carrying a word from the name dictionary (a possessive, "Grace's iPhone", counts as the name it is built on). With none of the three the column is recorded at `low` under `person_name` (or under a sub-threshold value signal's own category): the neighbouring-column rule's first arm still raises it beside a `likely` column, and the identical-name pass does not raise it from a same-named column in another table, because `users.name` holding people says nothing about `tags.name`. A lower rule that also matches still decides the column (`content_name` is `free_text`). A `*_file_name` column is outside the rule altogether (`rules.yml`'s `unless`), whatever its table. A person's name in a bare `name` column of a table not named for people, with samples the dictionary cannot carry — a script it does not hold, a name it does not list — is therefore copied: THREAT_MODEL.md T1's stated residual 3, which it already was for a non-Latin name in a column named in that script. Measured over the ten torture schemas before and after, 18 columns moved from masked to copied and none the other way, none of them personal; the three hand-labelled truth sets keep recall 1.000 and precision moves 0.629 → 0.641 (docs/TORTURE.md, T-0313). This is a recall change against THREAT_MODEL.md T1 and is reviewed as one there.

**Amended 2026-09-24 (T-0312, dogfood session 1): the identical-name pass does not treat a swept column as evidence.** The **Scoring** bullet's "identical column names across tables share a category" and the second arm above (T-0311's amendment, immediately below) are each correct in isolation and dangerous together: a column the second arm raised carries no name or value signal of its own about *its own* table, only "a certain neighbour sits beside it and nothing is known about its contents" — and the identical-name pass (`internal/classify`'s `sameColumnName`) used to treat that raise as ordinary evidence, exporting it to every same-named column in every other table regardless of whether that table has a `certain` neighbour of its own. Dogfood session 1's own run counted this as it happened: 25 propagations of exactly this shape, a source column's neighbour-swept decision carrying a category into a table it has no relationship to — `uuid` and `state` from one `devices` table into five tables each, `filename` as `credential` into three, `colour`, `country`, `role`, `ui_mode`, `type` and `title` into others — four of which were a text `uuid` column under a unique index, and each became a plan refusal over a column with no evidence behind it at all. `sameColumnName` now excludes a column the second arm raised from the set of columns it treats as a source (it may still be a *target*: a same-named column elsewhere with real evidence of its own still raises it, exactly as before). The first arm (a column with *some* signal of its own, raised `low` to `possible`) is unaffected — its category is evidence about a real, if sub-threshold, signal in this column, not a guess about a neighbour — and remains an ordinary source. This is a recall change against THREAT_MODEL.md T1: a same-named column that used to inherit a category from a swept source no longer does, so a column that was masked only on that inheritance is now left unmasked unless something else raises it (`propagateKeys` clearing `sweptNoSignal` when real FK-propagated evidence overwrites the sweep's guess recovers most of this, per the review round above). The package's two DB-free truth sets show no change either way — `TestPagilaPrecisionAndRecall` (precision 0.959, recall 1.000, n=64) and `TestFiftyNamesFromThreeSchemas` (precision 0.762, recall 1.000, n=87) — because neither hand-labelled set contains the swept-then-same-named shape this change affects; the torture-corpus measurement of the residual (a column swept, never FK-propagated, and no longer a `sameColumnName` source) is pending under T-0351.

**Amended 2026-09-24 (T-0311, dogfood session 1): the neighbouring-column rule's second arm, and what it spares.** The **Scoring** bullet above states the rule's first arm (`low` raised to `possible` beside a `likely` column). Since the 2026-09-15 red team (A2b) it has had a second: a character column with no name, value or type signal at all, in a table holding a column at `certain` under a category that identifies a person, is masked as `free_text` (`internal/classify`'s `unknownColumnsBesideCertain`; THREAT_MODEL.md T1's "rail under all of it"). On a production Rails schema of 143 tables that arm swept every such column of a users, devices or ads table — `role`, `ui_mode`, `os_type`, `state`, `log_level`, `timezone`, a text uuid, colours, asset paths — into filler where the application expects `admin` or `linux`, and the copy did not boot. Three things now spare a column from the second arm, each on its own reason line (`not swept by the neighbouring-column rule (<table> has N columns at certain): ...`), and the column is then copied exactly as one in a table with no `certain` neighbour is: (1) a unique index, which the arm always skipped but never said; (2) every non-NULL sample one **identifier shape** — a UUID, a hex digest (hex characters mixing digits and letters at a digest's length only: 8 to 12, an abbreviated commit hash, or exactly 32, 40, 64 or 128 where `textsig.LooksSecret` does not read the value as a secret; every other length is swept — T-0354, the T-0311 review, found a 15-character hex token spared under the first landing's 8-to-128 rule), a semantic version that is not also a dotted date with a four-digit year, a hostname (three labels, or two ending in a common suffix), a path (a `/`, no scheme, a segment with a letter, no month name beside all-digit segments, and a file extension, a leading `/` with three segments or more, or an IANA `Area/Location` zone name) — with at least `minSamples` (three) samples, and for the hostname and path shapes no value carrying a dictionary name or a special-category term; (3) an **enumeration** — at least 10 non-NULL samples, at most 20 distinct values, every distinct value seen at least twice, every value an ASCII token with no whitespace, and no value a dictionary name (CamelCase split first), a special-category term, a gender or title term, a blood group or marital status, a run of four digits or more (hyphens, dots, slashes and `+` between them allowed, so a ZIP+4 or a local phone number counts), or a date. The first arm is unchanged and so is FK propagation. The two sample-based spares are asked only after the sweep's own `fkPairs` pairing has agreed that the column's foreign-key partners could have been raised with it: a partner carrying a decision of its own still refuses the pair at exit 12 (T-0257), because that decision is evidence about the values on both ends of the join (`testdata/regressions/037`); a spared column raises nothing, so both ends of its join are copied in agreement (`testdata/regressions/031`, a currency-code pair that refused at exit 12 before and now loads). A column the identical-name pass later raises is still raised: the spare is a statement about this arm only. This narrows recall on a real shape, and THREAT_MODEL.md T1's T-0311 amendment carries the measurement over the torture corpus and the supabase-auth truth set, and the residual.
...
- **Generators consume only `h`:** index into embedded word lists; local parts from base32 of `h`; digits from `h`; the length of a free-text filler from `h`. No gofakeit. (Amended 2026-09-24, T-0328: a free-text filler's length now comes from the input's; every character of it still comes from `h`.)
- **Nothing survives:** no prefix, length, first character or real domain. Emails land in `example.com`/`example.net`/`example.org`; phones in the fictional `555-01XX` range or the corresponding reserved range for the detected region; IPs in documentation ranges; URLs under `example.invalid` with a hash-derived path. Free text is filler of a length drawn from `h` within `[1, min(atttypmod, 4096)]`, so a 1,247-character bio and a two-word note are indistinguishable after masking; `TestFreeTextLengthUncorrelated` asserts, for a fixed key, that output length does not correlate with input length. The two length-revealing exceptions are `NULL` and `''`, listed in §6 item 6. (Amended 2026-09-24, T-0328: free text is now a third, stated one — its filler is fitted to the input's length — and `TestFreeTextLengthUncorrelated` is retired; §5's T-0328 amendment below.)
...
**§5 amendment (2026-09-24, T-0328): free text is fitted to the input's length.**

Dogfood session 1 (docs/DOGFOOD_LOG.md) masked a users `role` column with three short values to three distinct 255-character paragraphs, and every device-state column (`os_type`, `log_level`, `timezone`, `state`) the same way, so the copy could not boot even where masking the column was right. The maintainer decided (T-0323) that for a short input `free_text` emits a value about as long as the input, not a column-width run of words. `mask`'s `freeTextMasker` now reads one thing from the input besides its shape — its length, the rune count of its canonical form (`fold`), so spellings that hash alike are fitted alike — and every character is still drawn from `h`:

- **A column that holds the longest filler word (nine characters) gets whole words**, one space apart, from the first word boundary at or past the input's length and never past `max(input length, 9)`: `admin` becomes one word, a sentence a run of words at most four characters shorter than it.
- **A narrower column gets exactly the input's length**, cut from the same words, because a whole word may not fit.
- **An input whose canonical form is longer than the column allows** (over the 4,096 cap in an unbounded `text` column, or a value NFKC or case folding lengthened past its own `varchar`, as ß becomes ss) keeps the column-width behaviour, a length drawn from `h` inside `[1, min(atttypmod, 4096)]`; an exact-length `CHECK` keeps its length, as before.
- **The first two draw from the filler words less ten an application is likely to store as a state or level of its own** (`default`, `input`, `level`, `output`, `session`, `stage`, `system`, `token`, `trace`, `value`), so a `log_level` column cannot mask one row's `debug` to another row's real `trace`; the last two draw from the whole list, unchanged.
- **Determinism and equality groups.** Same key, same canonical value, same column side of nine → same output. A canonical form no longer than the raw value fits every column that holds it, so the width decides nothing but which side of nine it is on: two foreign-key-linked columns holding one value mask it alike when both are at least nine characters wide (or both narrower), and the equality-group rule's fourth question — the same `Domain()` in every member — passes exactly then. The one exception is a value whose canonical form folding made longer than the narrower of two such columns (`straße`, canonically seven, in a `varchar(6)` keyed to a `varchar(8)`): the narrower takes the over-long branch, and the key does not validate at load.
- **`Domain()` is the narrowest length's count,** because the length now comes from the value and `Domain` is handed only the column: a one-character input is one of the 80 filler words it draws from in a column of nine or more, and one of their 21 first letters in a narrower one. So a free-text column's admissible domain is 80 or 21 whatever its width, where before it grew with the width (22,505 for a `varchar(255)`, 368,195 for `text`). **The unique-index rule's outcome does not change:** `d_required` is 500,000 at one row, so a unique `free_text` column was refused at plan (exit 12) at every row count before and still is; the refusal now prints `d = 80` (or 21) and the same largest `--take`, zero. **The small-domain rule's does, on paper:** a free-text column with more than 40 distinct sampled values is now `d < 2 × distinct` by the generator's figure, although `internal/core`'s `markSmallDomains` applies only the catalog half of the rule today (T-0341), so nothing is listed under `small_domain:` that was not before.
- **What it costs.** The approximate length of a free-text value survives a mask — a third stated length-revealing exception beside `NULL` and `''` (§6 item 6; THREAT_MODEL.md "Positions"). A short value masks to one filler word, so a source column that already holds one of those words (`batch`, `report`, `record`) can receive a masked value equal to one of its own source values, and the residual scan reports it as a hit: a known false-positive surface like the documentation ranges above, not a leak. Every masked `free_text` value changes, JSON string leaves included (they are masked as `free_text`, §4), so this is `mask/v0.5.0` and a release note; `special_category`'s text branch and `semi_structured`'s own string leaves still draw their length from `h` and do not change.
...
3. Verify streams every masked target column (every leaf of a masked JSON column), canonicalises each value, and tests the filter. On a hit it opens `Source.Short` (a new short `REPEATABLE READ READ ONLY` transaction on a fresh snapshot) and confirms in two probes, indexable form first: `SELECT EXISTS (SELECT 1 FROM t WHERE col = $1)`, then only if that is false `SELECT EXISTS (SELECT 1 FROM t WHERE lower(col::text) = lower($1::text))`. Confirmation probes are capped at 1,000 per run (`--residual-probe-cap`, printed with the count used); the cap exists because each case-folded probe can be a sequential scan on production and because a bound parameter lands in the source's log under `log_statement = 'all'` (THREAT_MODEL.md T4). A confirmed hit is exit 9 naming table and column. A hit the source says is absent is printed as `unconfirmed (filter false positive or source changed since snapshot)` and counted. A hit that **cannot be tested** — `Source.Short` cannot open, a probe errors, the role lacks `SELECT`, or the cap is reached with hits still untested — is exit 9 with `residual hits could not be confirmed: <reason>`, never a printed note, because an unconfirmable hit is the case where failing closed costs least. **Amended 2026-09-23 (ADR-015, proposed; T-0302):** for a column whose masker has a vocabulary (`mask.Emitting`, only `person_name` today), a hit whose value that vocabulary contains (`mask.Emits`) is tallied per canonical value instead of probed, and at the end of the column: a value the target holds more often than transform emitted it (`Residual.Emitted`) takes the two probes above on one retained target value, and a confirmed probe is exit 9 with `the target holds this value more often than the masker produced it`; then, where the step has a usable row identity (its identity columns when the plan knows them unique, a primary key or unique rung and never a pseudo-key, or a lookup's primary key; each unmasked, not generated and joinable), the rows holding a tallied value are fetched from the source by that identity, each batch of at most 1,000 sent as it fills during the scan so verify holds one batch per column (`verify.rowcheck.<table>`; a row-check statement binds only identifiers and joins on a unique, indexed key, so it does not spend `--residual-probe-cap`), and a row whose source value is canonical-equal or letters-and-digits-equal to its own target value is exit 9 with `the source row this row was copied from still holds this value in this column`. A row-check statement that errors, or a source that will not open, is exit 9 as an unconfirmable hit is; an absent row or an identity with a NULL part learns nothing and never falls back to the column probe. What survives is reported once per column as `verify.residual.explained`, a count and never a value. Every other hit keeps the column probe at once: every other category, a value off the list, a closed column, a custom, unknown or `fixed:` masker, and every JSON leaf.
4. **Second net:** the classifier's validators run over the full contents of every column of the loaded target that is not fully masked by a category masker: every unmasked, non-opted-out column, and the string leaves of every masked JSON column (their maskers were chosen per key by name, so the net checks the leaves). Canonicalisation and validator choice are driven by the column types in `Schema`, which `Verify` receives (§2). A column reaching `possible` is exit 9. The target is small, so this is a scan, not a sample. The second net re-runs the classifier's value validators at full coverage over the families it reads, so it catches a column the 200-row sample under-represented; it does not catch a category outside the rule pack, and T1 says so. Phase-4 gaps, tracked: eight of ten validators registered (T-0055), text families only (T-0050), strong threshold only, by decision (T-0056); THREAT_MODEL.md T1 states the same. **Amended 2026-09-24 (T-0319, dogfood session 1):** every failing column is reported, each as its own `verify.refused.second_net` line, before the run exits 9 — the net always scanned every column, but only the first failure was printed, so an operator met one column per run. The line names `--mask TABLE.COL=CATEGORY` with the category the net found, and `--skip-table`, so the answer to the net no longer has to be `--unmask` on a column it has just called personal. **Amended 2026-09-25 (T-0272):** on a masked `json`/`jsonb` column whose decision carries per-leaf categories (§4's amendment, and since T-0393 a column whose own name gives every leaf its category), the net leaves to the residual scan a string leaf the per-leaf rule says a category's own masker replaced — a fake email address is still an email address — and reads every other leaf and every key as before. The rule's value half holds every validator the net runs over a leaf, the phone validator under the run's `--phone-region` included, so a leaf the rule calls copied is one no net validator recognises, and a leaf it calls category-masked was recorded in the filter by transform (`internal/verify/jsonleaf.go`). ADR-015's rail for a generated column over masked columns only extends to the same case: when one of those columns is a masked document carrying per-leaf categories (a per-leaf map, or since T-0393 a name category whose leaf masker is its own), the net skips the validators whose categories a leaf's category masker emits, and runs the rest — `make torture` found the shape in Supabase's `auth.identities.email`, `lower(identity_data ->> 'email')`, which holds the email masker's output. **Amended 2026-09-25 (T-0397, the JSON red team's round 1, entry 24):** that skip is per value, not per column. The net reads the document columns such a generated column's expression names in the same target row, and skips a hit from a validator whose category a leaf's category masker emits only when the generated value equals, after `lower` and `btrim`, a string leaf of that row the per-leaf rule says a category's own masker replaced; every other hit counts. Before it, a generated column that assembled an email, a phone number or a card number out of copied leaves (`(identity_data ->> 'u') || '@' || (identity_data ->> 'h')`) passed the net at exit 0 beside the Supabase column the skip was written for. At plan time `internal/classify` gives every key a generated column's expression reads through `->` or `->>` from a `json`/`jsonb` column of its table the generated column's own category, in that column's per-leaf map where the map calls the key `none`, whenever a validator decides the generated column's samples, so those leaves are masked by transform rather than only caught here.
...
6. **Stated false negatives**, printed in the README and by `lazyslice doctor`, in this order: **the snapshot preserves production row identifiers, so anyone with any other reference to a production record (an admin URL, a ticket, a log line, a payment or support system) can re-identify every row exactly**, and the default `--take 500` copies the same lowest-id rows on every run; personal data in a column classified `none` that no validator recognises; a leaked value that was truncated, reformatted or embedded in a longer string (the filter tests canonical equality only); values inside `bytea`; JSON key names, which survive masking; a JSON leaf value that no name rule on its enclosing keys and no value validator recognises, under keys the samples showed, which is copied (§4's T-0272 amendment); `NULL` and `''`, which survive and reveal that much; the approximate length of a masked free-text value (§5's T-0328 amendment); masked columns with a small admissible domain, where the substitution is recoverable by frequency (§5); quasi-identifier combinations, among them, since T-0397, a generated column that assembles a personal value out of copied document leaves which no validator recognises either in its samples or in the target (item 4's T-0397 amendment masks the leaves when the samples validate and refuses the target value when it does); frequency and prefix leaks in unmasked columns; a masked value coinciding with another row's real value; values already fake in the source. **Amended 2026-09-23 (ADR-015, proposed; T-0302):** for `person_name` the coincidence with another row's real value is no longer only a false negative but an explained case (item 3), and a masked name never equals its own row's source value, because the masker is redrawn until it does not, which reveals about 1/500 of a bit per value. The one uncaught case narrows to a fault that both copied one row's listed real name into another row and lost the masked copy of that same name from the same column, on a table with no usable row identity.
...
`system_identifier` is appended as a fifth field **when the role can read it** — it is the strongest identity a cluster has and the only one that survives a restart — and `sameClusterIdentity` is **hierarchical, not a vote** (amended 2026-09-15, T-0190 fix round): when both sides filled that field it decides alone, equal for one cluster and unequal for two. Only when one side could not read it do the weaker fields decide, **field by field with any field either side left empty skipped**, never by string equality. That is what lets a superuser target and a `SELECT`-only source be compared at all, and what stops a postmaster restart between the source's read and the gate's — which moves the start time and leaves the system identifier alone — from making the gate stop recognising the source's own cluster. **Correction, 2026-09-17 (T-0241, round-4 red team): the identity does *not* work without `system_identifier` for every shape, and the claim above was false for one of them.** `EXECUTE` on `pg_control_system` is still not granted to `PUBLIC` by default; the recommended-role snippet below now grants it, and without that grant a source that is a streaming standby degrades the identity to "the postmaster start time and the server version" exactly as the paragraph above states — and the start time is the one field a standby and its own primary can never agree on, which is precisely what the amendment below closes. Granting it does more than "only strengthen" the comparison in that shape; it is what makes rule 1 able to prove the standby and its primary are one cluster at all.
...
**Amendment, 2026-09-16 (T-0222, R2-06's R3 replay: `docs/reviews/2026-09-15-redteam/round3-still-leaking.json`): the identity degrades field by field, and "neither identity could be compared" is answered as possibly the source's own cluster, not as different.** Two gaps, found by denying the source role `pg_postmaster_start_time()` too — a hardened cluster that revokes monitoring functions from `PUBLIC`, the same class of hardening the T-0190 amendment above already assumed for `pg_control_system` — and reaching one cluster again over a second transport, with the target database named differently from the source's own (the mid-migration shape: `app` reached directly, `newprod` over a second route to the same server).

First: the statement above read as one `SELECT` concatenating four fields with `||`, so a permission error on the one privileged call among them — `pg_postmaster_start_time()` — failed the *whole row*, and `Source.ClusterID` collapsed to `""` even though the maintenance database's oid and the server version needed no privilege that role lacked. Guarding the call with `CASE WHEN has_function_privilege(...) THEN ... ELSE '' END` inside that same statement looks like the fix and is not one: Postgres checks `EXECUTE` for every function call in a compiled plan at executor-initialisation time, before any `CASE` branch, `WHERE` clause or scalar subquery around it is evaluated, so a role denied the privilege still gets `permission denied` out of a statement that merely guards the call rather than omitting it — measured by hand against `postgres:16`. (`internal/pg`'s `TestGateRecognisesTheSourceClusterWithPostmasterStartTimeDenied` runs only the already-split statements end to end and asserts no permission error; it is not evidence for this claim. The regression pin is `TestTheClusterStartTimeIsGuardedByASeparateStatement`, which fails if the guard and the guarded call are ever recombined into one statement.) The guard has to be a *separate statement with no reference to the guarded function at all*, checked before the guarded statement is ever sent: `internal/pg` now reads the postmaster start time's own privilege, then the start time itself only when that check passed, then the other three fields (which carry no privilege check and are always sent) as a second statement, and joins the two. A role denied the one privileged field still contributes the rest, on both sides of the comparison — though, as the Third amendment below states, the two fields that survive under that role (the maintenance database's oid and the server version) are decisive only when they *disagree*: their surviving agreement proves nothing about sameness, only their disagreement proves the clusters different — whatever `EXECUTE` on `pg_control_system` is actually granted to by default — a question the T-0190 amendment above answers with a claim `internal/pg/CLAUDE.md`'s own T-0222 section found is not measured against any version and is owed a correction (**T-0224**).

Second: `Target.Gate`'s comparison used to fall back, when neither identity could be compared, to `targetRef.SameCluster(source)` — the two normalised endpoint spellings. That is exactly the comparison rule 1's identity check exists to replace: one cluster reached over two transports normalises to two different endpoints by construction, so falling back to it precisely when the identity check has nothing to go on answers "different cluster" from the one signal guaranteed to be wrong in that case. The R3 replay's differently-named target database also does not reach the fail-closed rule stated above this amendment — "unknown still refuses a target carrying the source's own database name" — because that rule is gated on `current_database()` equalling the source's own, and the mid-migration shape is outside it by construction. `Eligibility.SameCluster` is now `true` whenever nothing could be compared, the same direction `clusterUnknown`'s existing refusal already took for the narrower case it covers: the header prints `same cluster as source` as a warning (never a refusal on its own — rule 1's own text above is unchanged), and for a headless run whose target the ladder chose rather than the operator named, ADR-013's refusal below treats it exactly as a confirmed same-cluster answer. A false "possibly same cluster" costs one warning line or one refusal naming `--target`; a false "different cluster" is a production write.

**Third (T-0222 fix round, review): not every positional field is evidence of sameness, only of difference.** `sameClusterIdentity`'s field-by-field fallback treated any two fields that were present on both sides and equal as proof of one cluster, and the maintenance database's oid and the server version cannot carry that proof: from PostgreSQL 15 on, the oid is pinned to `5` for every cluster, and the version is shared by every cluster built from the same image — a role denied `pg_postmaster_start_time()` as well as `pg_control_system()` (the hardened role this section already describes) left exactly those two fields as the only ones either side could fill, so two genuinely unrelated clusters built from the same image compared equal on both and read as one cluster: `clusterUnknown()` went false where it should have stayed true, and rule 1's `SameCluster` warning fired on a legitimately separate target. **Agreement on the maintenance oid or the server version alone is not decisive; disagreement on either still is** — it takes no specificity to prove two clusters apart, only to prove them the same, and the start time and `data_directory` are the fields specific enough to a running cluster to carry that proof. When the only fields either side could fill are the oid and the version and they agree, the comparison now reports unknown, the same fail-closed direction as no common field at all, rather than a confident "same cluster" the fields do not support.

**Fourth (T-0241, 2026-09-17, round-4 red team: `docs/reviews/2026-09-15-redteam/round4-still-leaking.json`, "a read replica"): the start time is specific enough to prove sameness but not difference, and the Third amendment above claimed the opposite for it.** `--source` a streaming standby, `--target` a database on that standby's own primary, under the recommended `SELECT`-only role with no `pg_control_system` grant. A primary and its hot standby share `system_identifier` but a standby's postmaster necessarily started later than its primary's — it started when `pg_basebackup` finished — so the start time *always* disagrees between them, exactly the way the maintenance oid and the server version can agree between two unrelated clusters built from the same image. Before this fix the start time was treated as decisive on disagreement the way `data_directory` still is, so with `system_identifier` unreadable on the source side this one field read "different cluster" with total confidence, and the run wrote a masked slice into the standby's own primary with no `same cluster as source` warning printed at all. The start time now behaves the mirror image of the maintenance oid and the server version: **agreement on it remains as decisive as ever (two independent postmasters starting in the same microsecond is not a case), but disagreement on it alone, with `system_identifier` missing on either side, downgrades the verdict to unknown rather than a confident "different"** — the same fail-closed direction `clusterUnknown` already takes for "nothing comparable at all", read in the direction the Third amendment's own reasoning already argued for the oid and the version: a field can be specific enough to prove sameness without being specific enough to prove difference. `data_directory` is unaffected and still decides on disagreement, because a standby's own data directory is not constrained to disagree with its primary's the way the start time is.

**The cost is not confined to a standby, and is stated here rather than left for a reader to work out (T-0241 fix round, review).** Under the recommended role, on PostgreSQL 15+, `system_identifier` and `data_directory` are both already unreadable, so the only fields either side can ever compare are the maintenance oid and the server version — both `clusterIDWeakField`, both provable only on disagreement, never on agreement. Discounting the start time's disagreement unconditionally, rather than only when nothing else stands in for it, means **two genuinely unrelated clusters that happen to share a Postgres major now read as unknown too, wherever they were previously distinguished by the start time alone** — not only a standby against its own primary. That is the ordinary shape of a production `--source` and a locally built `--target` of the same major: `sameClusterIdentity` answers unknown, `Eligibility.SameCluster` reads `true`, an interactive run prints a false `same cluster as source` warning, and a **headless run with a ladder-chosen target** (`lazyslice --yes` with no `--target`, the CI shape) is refused at ADR-013's escalation until the operator names `--target` or applies the `pg_control_system` grant this section recommends. This is accepted rather than fixed: a false "possibly same cluster" costs a warning line or a clearable refusal, and a false "different cluster" is the production write this whole rule exists to stop, which is the same fail-closed trade the Third amendment already made for the oid and the version. `internal/pg`'s `TestGateReadsTwoUnrelatedSameVersionClustersAsPossiblySameUnderTheRecommendedRole` (`gate_integration_test.go`) pins this cost against two real, independently created containers of one image, restricted to the recommended role.

This alone does not refuse the write — an unknown identity is `Eligibility.SameCluster = true`, "possibly the same cluster", which is a warning for an operator-named `--target` and ADR-013's existing headless escalation for a ladder-chosen one — so a second, independent rail closes the gap ADR-013's own cheap `clusterKey` check cannot reach: a standby and its primary ordinarily publish on *different* ports, so the address-only comparison that check and the tie-break above both use never flags them as one cluster in the first place, and the round-4 red team's second reproduction reached exactly that path (a target discovered through `$DATABASE_URL`, not named by the operator). `internal/pg`'s `Source.Replica` reads `pg_is_in_recovery()` — executable by `PUBLIC` on every supported version, needing no grant at all — and, when it answers true, `pg_stat_wal_receiver`'s `sender_host`/`sender_port` where the role may read them. The source stage now prints a header line whenever the source is a standby, interactive or not (`source.warn.standby`, naming the primary when the sender columns were readable), and a **headless run with no `--target` refuses outright**, exit 4, naming `--target` (`source.refused.standby_no_target`) — independent of `Eligibility.SameCluster`, because the standby rail and the identity comparison rest on different facts: the sender-address comparison and the `data_directory` carve-out (Fifth amendment, below) both depend on what the source role can read, and either can be unreadable on a role that still lets the identity comparison run, so the standby rail stays in place rather than being subsumed by it. A `--target` the operator named, on the standby's primary or anywhere else, is unaffected by either rail, exactly as ADR-013's own escalations already work; see ADR-013's amendment recording this as its third escalation.

**Fifth (T-0255, 2026-09-17, round-5 red team: `docs/reviews/2026-09-15-redteam/round5-still-leaking.json`, "the standby data_directory variant"): "`data_directory` is unaffected and still decides on disagreement" was the Fourth amendment's own claim, and it is wrong for exactly the case that amendment exists to cover.** A co-hosted standby and primary — `pg_basebackup` into a second directory beside its source, the ordinary way a standby is stood up alongside its own primary — disagree on `data_directory` by construction, the same way a standby's postmaster start time always disagrees with its primary's. A source role that can read `data_directory` is not a hardened-role edge case either: it is `pg_read_all_settings`, a monitoring-grade read-only grant, and precisely the class of grant a DBA hands out to make the recommended snippet's *other* reads work. Under such a role `data_directory` was still `clusterIDDifferenceUnreliableField`'s one field short — decisive on disagreement exactly as any other non-weak field is — so a standby whose primary is a named `--target` read as "different cluster" with total confidence and no `same cluster as source` warning printed, the round-4 finding reopened through a second field. `data_directory` now joins the start time among the fields `sameClusterIdentity` discounts on disagreement, but **only when the source is a standby** (`Source.Replica`'s own `Standby` answer, already read for the header line and the headless refusal above): an ordinary, non-standby source has no reason at all to share a data directory with an unrelated cluster, so `TestADataDirectoryDisagreementStillProvesTwoClusters` (`internal/pg/cluster_test.go`) still requires that case to stay decisive, and widening the carve-out unconditionally — discounting every field's disagreement regardless of what the source is — would make that test pass for the wrong reason.

A second, cheaper check closes the same gap more directly, for the source role that can read `pg_stat_wal_receiver`'s `sender_host`/`sender_port` (`pg_read_all_stats`, or superuser): when the source is a standby and its WAL receiver's sender resolves to the target's own `host:port` — `dsn.Ref.SameCluster`, the same normalised host:port string comparison rule 1's own endpoint checks already make — `Eligibility.SameCluster` is reported `true` outright, ahead of and independent of the identity comparison above. **This is an opportunistic extra rail, not an alias-proof one**: `SameCluster` is normalised-string equality on host and port, with no DNS resolution and no container-alias lookup, so it fires only when the sender address `pg_stat_wal_receiver` reports is spelled the same way the target DSN is — e.g. both as `primary.internal:5432`, or both as the same published port. It does not fire when the two are reached by different routes to the same server — a container reached at a published Docker port (`127.0.0.1:15610`) as the target while `primary_conninfo` spells the sender as the container's own name and internal port — and in that shape only the identity comparison and the `data_directory` carve-out above are left to catch it. Nothing before this amendment compared the sender address with the target at all: `Source.Replica` read it and the source stage printed it into `source.warn.standby`'s header line, and the value was discarded there. `internal/pg`'s `senderMatchesTarget` (`target.go`) is the comparison, and `Target.Gate` runs it beside `sameClusterIdentity`'s own answer, from a `ReplicaStatus` `internal/core`'s `openTarget` hands the target the same way it already hands over the source's cluster identity (`Target.SetSourceReplica`, alongside `SetSourceCluster`).
...
**Amendment, 2026-09-16 (T-0184, ADR-013, proposed): a headless run's tie-break never runs at all when every candidate it would rank is on the source's own cluster.** The 2026-09-15 red team pointed `--source` at production with no `--target` and `--yes`; the ladder's only target-shaped candidate was another database on the source's own server, the tie-break took it as the sole entry, and the run wrote there under exit 0. Rule 1 above still makes a second database on one cluster *eligible* — that is unchanged, and an interactive run still reaches the tie-break, the decision header and the `same cluster as source` warning exactly as before, because a person watching it can stop it. What changes is headless only: a headless run — `--yes`, or no controlling terminal: the run cannot ask, the same predicate `internal/discover`'s `prompterFor`/`isHeadless` already use, not `--yes` alone — with no `--target` given (by flag, positional DSN or committed `lazyslice.yml`) refuses at exit 4, naming `--target`, before the tie-break runs, when the target-shaped set is non-empty and every member of it shares the source's cluster (the same address-based `clusterKey` the tie-break itself already compares on, not the gate's live `system_identifier` check, which needs a connection this step does not have). A CI job or cron entry that simply omits `--yes` has no controlling terminal either and is headless by this same rule — that is the red team's own shape. A `--target` named explicitly on the source's cluster is still eligible; this narrows what the ladder may pick unsupervised, not what an operator may name. See ADR-013.

A second, later check exists beside this one: the gate's own `Eligibility.SameCluster` (rule 1, above) is the authoritative signal — `system_identifier`, or cluster identity/endpoint as a fallback — and it can be true for a target this step's cheap `clusterKey` address comparison did not catch (a pooler, an SSH tunnel, `host.docker.internal` vs `127.0.0.1`). `internal/core`'s `openTarget` refuses there too, headlessly, exit 4 (`target.refused.gate_same_cluster`), for a target the ladder chose and this step's check missed; it never fires for a target the operator named — by flag, positional DSN, or a committed `lazyslice.yml` — exactly as this step does not. See ADR-013's Decision, second bullet from the bottom.
...
**Amendment, 2026-09-25 (T-0333/T-0385): the container name is `lazyslice-target-<project>` with one leading `lazyslice-`/`lazyslice_` segment of `<project>` stripped, and the function is not injective.** A directory named `lazyslice-<something>` gives a compose project name that already starts with `lazyslice-`, and prefixing that unchanged would read `lazyslice-target-lazyslice-<something>` — a real case, not a hypothetical one: the maintainer's own dogfood directories are named `lazyslice-dogfood*`. `provision.Name` strips one such leading segment from the project before adding the prefix, so `lazyslice-dogfood3` names `lazyslice-target-dogfood3`, never the doubled form; `lazyslice.yml`'s own `service:`/`target_label` field still records the *project* name untouched, since only the container name this function builds is affected. This makes `Name` **not injective**: two different projects — `shop` and `lazyslice-shop`, or two checkouts named `foo` and `lazyslice-foo` — collide on one container name, `lazyslice-target-shop` / `lazyslice-target-foo`. Nothing here resolves that collision; `--create-target`'s own-container check and Q1's `reuseOwn` (the T-0334 amendment below) are what refuse a same-named container that belongs to another directory's project, by comparing `lazyslice.working_dir` as well as the name.

A `lazyslice.yml` committed before this stripping existed recorded the unstripped name — `lazyslice-target-lazyslice-foo` for project `lazyslice-foo` — as its `target_label`, and the container itself, and the password `--create-target` remembered for it, still carry that name; `rung0Target` (`internal/discover/discover.go`) recognises both the current and this legacy name when matching a recorded `target_label` and when reading back the remembered password, so a second run against such a file does not regress to asking for a password that used to need none (T-0385).

**Amendment, 2026-09-24 (T-0327, ADR-016, proposed): a committed `target:` record of lazyslice's own container names the container, not its port.** A record with `from: container`, a `service:` shaped like `lazyslice-target-<project>` and a loopback host is resolved by name on a local, reachable Docker endpoint, and only a container that carries the `lazyslice.project` label counts. Running, the target is its live binding with the role and password from its own environment. Stopped, it is Q1′. Absent, it is Q1 with a first sentence naming the missing container, and headless it stops at exit 4 (`target.refused.container_missing`) naming `--create-target` and `--target`. `--create-target` outranks such a record and provisions or reuses this directory's own container. Every other record, and this one when Docker cannot answer, is still the address rung 0 made it (ADR-008 §1). A target that answers and refuses the password stops at exit 4 under `target.refused.auth`, naming the role and the four places a password comes from, never as a bare SQLSTATE. The gate is unchanged.

**Amendment, 2026-09-24 (T-0334, dogfood session 3): the next run chooses the container this directory's first run created.** Provisioning creates `lazyslice-target-<project>` with `POSTGRES_DB=postgres`, and the 2026-09-15 red team's rule keeps a maintenance database off the target tie-break, so the second run from the same directory listed its own running, empty container and asked Q1 again for the same name on the next port (headless: exit 4, `target.refused.none`). A running container whose `lazyslice.project` label is this directory's project and whose `lazyslice.working_dir` label is this directory (not only one with the same basename, such as a second checkout or a worktree) is now ranked like any other target-shaped candidate even though its database is `postgres`; ADR-008 §5's order is unchanged and the gate judges it exactly as before. The "probably empty" hint is the connected database's own `pg_class` count, not a statement about other databases in the cluster, and a load into `postgres` writes nothing else, so no second probe is added to the dial. Q1 never proposes a container name that already exists: when `lazyslice-target-<project>` exists and the ladder could not rank it, the run starts it with Q1′ if it is stopped, reuses it with no question if it is running and ours, and stops at exit 4 (`target.refused.name_taken`, naming `--target`) if it does not carry both labels for this directory (somebody else's container, or another checkout's of the same name), without reading or starting it. For a committed record whose container is missing (ADR-016), the same reuse applies at a terminal only; headless keeps ADR-016's `target.refused.container_missing`.

**The question rule** (ADR-017, proposed 2026-09-24, T-0343, narrowing ADR-008 §5): lazyslice asks no question whose default the operator has already been shown. At a controlling terminal it asks every item on the ladder source → target → root table → row count → masking that it could not determine with confidence, in that order, one question per item, so a first run with both the target and the root open asks Q1 (or Q1′) and then Q2; every settled item takes its computed default and is printed as a decision with the flag that changes it; an item with no safe default and nobody to ask is not asked, the run stops and prints each probe and the command to run instead. A headless run asks nothing. This replaces the one-question rule taken from research/SQLIT_STUDY.md §5.3 ("at most one blocking question per run"), which let Q1 take a first run's only question and left the root to a silent default (docs/DOGFOOD_LOG.md, session 3).
...
columns:
  ...
  public.feed_items.payload_url:
    category: online_id
    confidence: certain
    reason: "raised by lazyslice.yml for this column"
    masker: online_id
    mask:                          # --mask, recorded; masked on every run until removed
      category: online_id          # free_text for the bare --mask TABLE.COL
      by: flag                     # or the name of whoever asked, for a hand-written entry
...
types:                                # --allow-type-literal, recorded; the same shape unmask has for a column
  public.assignee:
    reason: "product labels, checked by hand"    # never empty; --allow-type-literal TYPE=REASON
    by: flag                                     # or the name of whoever approved it, for a hand-written entry
    type: 7c2a9e10                # type fingerprint (enum labels, or a domain's definition) at decision time
...
The rule, and it is one rule with four arms:
...
4. **An enum's labels, and a domain's `DEFAULT` and `CHECK`, belong to a *type* rather than to a column** — item 3 above recreates them verbatim too, as `CREATE TYPE ... AS ENUM (...)` and a domain's whole `CREATE DOMAIN` statement — and neither can be rewritten by any of the three arms above: every row of every column of an enum type references a label *by value*, so a masked label would either break the column or silently remap rows, and a domain's `DEFAULT`/`CHECK` is declared once for a type that may back several columns under several different categories, or none, so there is no single masker whose output is the right replacement. A literal there that a strong validator hits is therefore always **exit 13**, `target.schema.type_literal`, naming the type and the position of the label or the definition — never the label text (T4) — whether the columns that use the type are masked or not. The escape is **`--allow-type-literal TYPE=REASON`**, §8's per-column opt-out spelt for the one object class that is not a column: a reason is required for the same purpose `--unmask` requires one, the name is resolved against the source's own enums and domains so a typo is exit 2 rather than a rail the operator believes they lifted and did not, and it is recorded under the yml's own `types:` block (§10) on `--unmask`'s tighten-only rule (ADR-004) — reason required, expiring when the type's labels or definition change. `--skip-table` has no effect on this refusal: it drops a table to *schema only*, which still recreates that table's DDL and every type the DDL names, so a source schema carrying one such label was permanently unrunnable before this escape existed (the T-REDFIX review's fourth finding). `internal/verify`'s catalog pass honours the identical opt-out, through `Plan.AllowedTypeLiterals`, so the run that gets past the planner is not refused at exit 9 over the object the operator was told they had allowed.
...
**Amended 2026-09-16 (T-0198, the round-3 red team, docs/reviews/2026-09-15-redteam/round3-still-leaking.json finding 15): `special_category` has a value validator.** The previous amendment's own admission — that R2-09's special-category sentence was closed only by accident, caught solely because a date happened to supply `AddressShape`'s digit — is what round 3 confirmed live, twice: `CHECK (diagnosis <> 'HIV positive, CD4 210')` and `CHECK (religion <> 'Ahmadiyya Muslim')`, on two columns `internal/classify` had already masked *by name alone* (§4's own rule for this category), both short of every shape the other ten validators ask for and both recreated into the target's `pg_constraint` verbatim under exit 0. `textsig.SpecialCategoryVocabulary` is a small, precision-over-recall term list for the six categories `rules.yml`'s own `special_category` name pattern already names — health condition or diagnosis, religion or belief, sexual orientation and gender identity, ethnicity or race, trade-union membership, political opinion — scored over a literal's text the way the other nine validators already are. It joins `strongValidators`/`strongHit` (`internal/plan/ddlliteral.go`), `strongCatalogHit` (`internal/verify/catalog.go`) **and, since the T-0198 fix round below, `internal/verify/validators.go`'s row-scanning second net** as an eleventh entry, so a literal or a row value that carries one of its terms is refused with the category named, exactly as an email or a phone number already is. This alone closes both round-3 canaries and needs no `--unmask` curation of any real schema: neither canary depends on the wider rule the same task's tracker log first proposed alongside it (below).

**The wider rule the tracker log proposed alongside it — refusing a masked column's own `CHECK`, generated expression, index predicate or non-rewritable `DEFAULT` on *any* literal, whatever it parses as — landed only for a `DEFAULT` and a generated expression, never for a `CHECK` or an index (T-0198 fix round, 2026-09-16, re-measuring finding 1 against `make torture`).** The first landing applied it everywhere, measured the cost on the ten real schemas in docs/TORTURE.md, found more than the two the tracker task's own decision named as the line for landing it as default rather than behind the existing `--unmask` escape, and — re-curating gitlab, odoo, discourse and supabase-auth one refusal at a time rather than stopping at the first count — found the cost was not only large but in two cases genuinely unsafe. A `CHECK` or an index can name any number of columns, and `namedColumns` only ever reports which ones a definition's *text* mentions, never which one a given literal is actually about: odoo's `res_partner_check_name` (`CHECK ((type = 'contact' AND name IS NOT NULL) OR type <> 'contact')`) names both `type` and `name`, and the only escape the rule could offer for the literal `'contact'` — which is about `type` and never about `name` — was `--unmask public.res_partner.name`, the real person's name Odoo's own CRM contacts table exists to hold. `res_partner_mobile_partial_gin_idx`'s trigram expression, `regexp_replace(mobile::text, '[\s\\./\(\)\-]', '', 'g')`, names one masked column, `mobile` — a genuine phone number — and its non-empty literals are a punctuation character class and a regexp flag, neither a value about the phone number at all; the only escape was `--unmask public.res_partner.mobile`, the phone number itself. Curating either would leak real personal data under a flag whose entire purpose is to say a column carries none, which is a correctness defect in the rule and not merely a curation cost. A `DEFAULT` and a generated expression carry no such risk — `internal/plan`'s `named` is always the single column itself there, never ambiguous about which one a literal is about — so the wider rule stays exactly where the risk does not: `internal/plan/ddlliteral.go`'s `columnDefault` and the generated-expression call into `fixedExpression` (`broadNet` true), never the constraint or index calls (`broadNet` false); `internal/verify/catalog.go`'s `unrewritableKind` answers `kindDefault`/`kindGenerated` only. A `CHECK` and an index still refuse on a strongHit exactly as they always have — that narrower, escape-free-when-masked refusal is this section's original rule and does not change — only the newer net that refused on a literal no validator recognised at all is gone from those two object classes. `make torture` passes all ten schemas at the original twenty-seven flags with this scope, which needed none of the curation the first landing's own measurement called for.

The exemptions the wider rule still carries, on the object classes it still reaches, are unchanged: a Pattern operand (`StripPatternMeta`-reduced, a fragment of a shape rather than a value in its own right — `testdata/nasty.sql`'s own `CHECK ("EmailAddress" LIKE '%@%.%')` is a `CHECK`, not a `DEFAULT`, and was never refused by this rule regardless), a closed value list (`mask.Constraints.Checks` already reads it to mask the column's *rows*), an empty collection literal (`'{}'`/`'[]'`, §5's own rule for the row side), and, since the same fix round, a literal immediately cast to a non-text type (`'-1'::integer`, the sentinel shape a `COALESCE` guard against a NULL foreign key commonly takes) — the deparser's own cast is what proves the value can never be free text, whatever category a shape guess would otherwise reach for.
...
**Amendment, 2026-09-17 (the gate-5 audit of this list).** The last item is split, and its second half moves past v1. What shipped: the classifier walks a JSON document's leaves (to sixteen levels, not one), matches each key against the rule pack and each value against the validators, and a hit decides the column; every string leaf of a masked JSON column is then masked, and a key that itself parses as an address, a number or a card goes through that category's masker (T-0137). What did not ship: a category *per leaf* on the `Decision`, which is what would let an email leaf become a fake email rather than filler (§5's note of 2026-09-06). It touches no control — coverage is the same either way, only the shape of the fake differs — and it waits on the maintainer's arbitrary-JSON policy decision (T-0143): under whole-document replacement there are no leaves left to categorise, so building it first risks building it to delete. It is T-0272 in E9 with the rest of the deferred list. The same audit found two items on this list and in the accepted ADRs that had not shipped at all; both landed inside phase 5 the same day: the network-namespace test is `make egress` and the blocking CI job `egress` (`tools/egress/run.sh`, T-0270, THREAT_MODEL.md T4 has what it proves and what it does not), and ADR-008's root-table question, Q2, is asked by `internal/core` after introspect and before plan (T-0271).

**Amendment, 2026-09-25 (T-0272).** The half of the item above that moved past v1 has landed, after the maintainer's arbitrary-JSON decision (T-0143, 2026-09-24: per-leaf classification, not whole-document replacement). Its premise changed on the way: the 2026-09-17 text said it "touches no control" because every string leaf stayed masked, and the decision it waited on is what made it touch one — a leaf with no signal is now copied, which is §4's amendment, §6 item 6's new false negative and THREAT_MODEL.md T1's T-0272 amendment. It is no longer on the deferred list.
