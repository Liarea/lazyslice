...
-- off the end of this file and issues its two statements itself when big is
-- set. The 2,000,000-row fill is gated because every test that is not about
-- streaming pays for it otherwise.
--
...
--   alt_emails     text[] of email addresses
...
-- knows none of that. lazyslice must print
--
--     polymorphic pair detected, not followed: no constraint
--
-- and leave the rows out (ARCHITECTURE.md section 14). What it must never do is
-- follow the pair silently, and what it must never do instead is stay quiet:
-- research/COMPLAINTS.md FK-10 is the silently empty slice this trap exists to
-- make impossible.
--
-- attachment 839 points at a row that does not exist, so an implementation that
-- does follow the pair one day still has to survive a dangling owner.
--
...
INSERT INTO public.people
    (person_id, manager_id, given_name, family_name, email_verified, ref, status, contact, alt_emails, notes)
OVERRIDING SYSTEM VALUE
VALUES
    (90000, NULL, 'Ada', 'Lovelace', true, 'ada.lovelace@example.com', 'active',
    ...
    (90007, 90000, 'Grace', 'Hopper', false, 'grace.hopper@example.com', 'active',
    ...
    (90014, 90000, 'Alan', 'Turing', true, 'alan.turing@example.com', 'suspended',
    ...
    (90021, 90007, 'Katherine', 'Johnson', true, 'katherine.johnson@example.com', 'pending',
     '{"profile": {"contact": {"email": "katherine.johnson@example.com", "phone": "+1 757 555 0188"}, "locale": "en-US"}, "tags": ["reviewer"]}'::jsonb,
     ARRAY['kjohnson@example.org', 'katherine@example.net'],
     'Katherine Johnson is the reviewer of record for Alan Turing.'),
    (90028, 90007, 'Edsger', 'Dijkstra', false, 'edsger.dijkstra@example.com', 'closed',
...
INSERT INTO public.tenant_users (tenant_id, user_id, email, joined_on) VALUES
    (1, 1, 'ada.lovelace@example.com',      '2024-01-05'),
    (1, 2, 'grace.hopper@example.com',      '2024-01-06'),
    (2, 1, 'alan.turing@example.com',       '2024-02-11'),
    (2, 7, 'katherine.johnson@example.com', '2024-02-12');
...
INSERT INTO public.tenant_user_sessions (session_id, tenant_id, user_id, started_at, client_ip) VALUES
    (5000, 1, 1,    '2024-05-01 12:00:00+00', '203.0.113.7'),
    (5011, 1, 2,    '2024-05-01 12:05:00+00', '203.0.113.8'),
    (5022, 2, 1,    '2024-05-02 08:30:00+00', '198.51.100.22'),
    (5033, 2, 7,    '2024-05-02 08:31:00+00', '2001:db8::1'),
    (5044, 2, NULL, '2024-05-03 07:00:00+00', '198.51.100.23');
...
INSERT INTO public.organisations (organisation_id, name) VALUES
    (500, 'Analytical Engines Ltd'),
    (503, 'Compiler Works');
...
INSERT INTO public.attachments (attachment_id, owner_type, owner_id, filename, uploaded_by) VALUES
    (800,  'people',   90000, 'signature.png',   'ada.lovelace@example.com'),
    (813,  'people',   90007, 'id-scan.pdf',     'grace.hopper@example.com'),
    (826,  'projects',   700, 'spec-v3.pdf',     'alan.turing@example.com'),
    (839,  'people',   99999, 'orphan.txt',      'nobody@example.invalid');
...
INSERT INTO public."LegacyCustomer" ("CustomerID", "EmailAddress", "MobileNumber", "Notes")
OVERRIDING SYSTEM VALUE
VALUES
    (42, 'ada.lovelace@example.com',  '+44 7700 900123', 'Migrated from the 1998 system. Contact is Ada Lovelace.'),
    (43, 'grace.hopper@example.com',  '+1 415 555 0132', 'Do not merge with Grace Hopper''s new record.'),
    (44, 'alan.turing@example.com',   NULL,              'Left blank on purpose.');
...
INSERT INTO public.audit_log (entry_uid, person_id, action, occurred_at) VALUES
    ('AE-0001', 90000, 'login',           '2024-05-01 12:00:00+00'),
    ('AE-0002', 90007, 'login',           '2024-05-01 12:05:00+00'),
    ('AE-0003', 90014, 'password.reset',  '2025-02-01 09:00:00+00'),
    ('AE-0004', 90021, 'account.review',  '2025-02-02 09:00:00+00');
