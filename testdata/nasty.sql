...
-- the undeclared pair. It is what puts these rows inside the slice, and that
-- makes the trap stronger rather than weaker: the row is selected, the
-- polymorphic parent is still not followed, and the run still has to say so.
-- It is nullable, and attachment 839 leaves it NULL, so the dangling
-- polymorphic owner is still in the fixture on a row the slice does not reach
