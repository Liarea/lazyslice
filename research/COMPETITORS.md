...
|---|---|---|---|---|---|
| [Greenmask](#greenmask) | Go | yes | yes | no | active |
| [Basecut](#basecut) | closed binary | yes | yes | yes | launched 2026, releases paused since March |
| [dbslice](#dbslice-2026) | Python | yes | yes | yes | new 2026, most direct OSS rival |
...
| [PostgreSQL Anonymizer](#postgresql-anonymizer) | Rust + PL/pgSQL | no | yes | yes (`anon.detect`) | very active |
| [Jailer](#jailer) | Java | yes | filters only | no | very active, GUI-first |
| [condenser](#condenser) | Python | yes | no | no | **stale** — last commit 2023 |
| [Replibyte](#replibyte) | Rust | yes | yes | no | **dead** — last commit 2024, won't build |
| [pgsubset](#pgsubset) | Rust | yes | no | no | **dead** — one day of commits, 2022 |
| [pg_subsetter](#pg_subsetter-teamniteo) | Go | yes (fraction) | no | no | low activity |
| [pg_sample](#pg_sample) | Perl | yes | no | no | low activity |
| [Tonic Structural](#tonic-structural) | closed | yes | yes | yes | commercial, enterprise-priced |
| [Redgate TDM / Data Masker](#redgate-data-masker-and-redgate-test-data-manager) | closed | TDM only | yes | yes | commercial, per-TB |
| [fixturize](#fixturize-2026) | Go | yes | yes | yes | new 2026, quiet since May |
| [pgEdge Anonymizer](#pgedge-anonymizer-2025) | Go | no | yes | no | new 2025, active |
| [Brume](#brume-2026) | Java | yes | yes (pseudonymisation) | no | new 2026 |
| [pg_partialcopy](#pg_partialcopy-2025) | Go | yes (hand-written SQL) | yes (in SQL) | no | new 2025 |
| [slice-db](#slice-db) | Python | yes | yes | no | low activity |
| [msg555/subsetter](#msg555subsetter) | Python | yes | yes (faker) | no | active, low profile |
| [DBSnapper](#dbsnapper) | closed binary | yes | SQL script | no | commercial |
| [db-condenser](#db-condenser-fork-of-condenser) | Python | yes | no | no | fork, new 2025/26 |

Adjacent but not competing: [Seedfast](#adjacent-not-competitors), [VeilStream](#adjacent-not-competitors), [Xata](#adjacent-not-competitors), [Ardent](#adjacent-not-competitors).
...
- **Subsetting:** You write `subset_conds` on a table in the dump config. Greenmask introspects the schema, builds a table dependency graph, and — per the [Database subset docs](https://docs.greenmask.io/latest/database_subset/) — "generates queries for subset conditions based on the introspected schema using joins and recursive queries". Traversal is **bidirectional**: filtering a parent pulls in the children that reference it, and pulls up the parents needed to keep FKs satisfiable.
- **Cycle handling:** Recursive SQL. The docs state the limitation plainly: Greenmask "can resolve multi-cycles in one strongly connected component, but only for one group of vertexes" ([database_subset](https://docs.greenmask.io/latest/database_subset/)). The remaining case is tracked as [#197 "Advanced Subset: Multi-cycles with more than one group in one SCC"](https://github.com/GreenmaskIO/greenmask/issues/197). Missing FKs are declared by hand as `virtual_references`, and polymorphic associations via `polymorphic_exprs`.
...
- **Last release:** `v0.1.21`, 2026-03-26. No public release in the five months to September 2026, and the release repo's last push is the same date. **Whether development continues privately is unverified.**
...
**Three most-upvoted open issues:** none — the tracker has one open item and it is a pull request. The closed issues are the useful signal: [#8 `--stream` + `--out-file` produces empty output after successful extraction](https://github.com/nabroleonx/dbslice/issues/8) (fixed 2026-05-20) and [#1 Schema selection option](https://github.com/nabroleonx/dbslice/issues/1) (fixed 2026-02-28). `pyproject.toml` still classifies it "Development Status :: 3 - Alpha".
...
- The acquisition closed 2025-08-01 and was announced 2025-09-25 ([Crunchbase acquisition profile](https://www.crunchbase.com/acquisition/grow-therapy-acquires-neosync-cd81--00632527), [Grow Therapy press release](https://www.prnewswire.com/news-releases/grow-therapy-raises-the-privacy-bar-in-mental-health-302567153.html), [Pulse 2](https://pulse2.com/grow-therapy-acquires-data-privacy-company-neosync/)).
...
- Snaplet the company shut down on 31 August 2024, open-sourced its tooling and the team joined Supabase ([Supabase: "Snaplet is now open source"](https://supabase.com/blog/snaplet-is-now-open-source), [HN discussion, 2024](https://news.ycombinator.com/item?id=41244171)). The original announcement post is only reachable through the [Wayback Machine](http://web.archive.org/web/20240716025550/https://www.snaplet.dev/post/snaplet-is-shutting-down): **`snaplet.dev` no longer resolves in DNS at all** (checked 2026-09-05). Two years after the shutdown, the company's own domain has lapsed.
...
- **Licence:** [The PostgreSQL License](https://gitlab.com/dalibo/postgresql_anonymizer/-/blob/latest/LICENSE.md), © 2018–2026 DALIBO SCOP.
- **Last commit:** 2026-09-04 (three commits that day). Current version 3.1.3; release 3.2 in flight ([#674](https://gitlab.com/dalibo/postgresql_anonymizer/-/work_items/674), 2026-09-04).
- **Stars:** 294 · **Forks:** 109.
...
- **CLI:** the usage string is emitted from source at [`CommandLineParser.java` L122–143](https://github.com/Wisser/Jailer/blob/master/src/main/engine/net/sf/jailer/CommandLineParser.java#L122-L143):
  ...
  ```
  Note the first positional argument: **`<extraction-model>`**. The CLI cannot subset anything without a model file, and the shipped [`extractionmodel/`](https://github.com/Wisser/Jailer/tree/master/extractionmodel) directory contains only two `.jm` demo models. The tool for authoring one is the GUI editor. `jailer.sh` itself is [a nine-line classpath wrapper around `java -cp … net.sf.jailer.Jailer`](https://github.com/Wisser/Jailer/blob/master/jailer.sh) — you also supply your own JDBC driver jar.
...
- [#126 Subject condition is violated due to reverse traversal (unexpected rows in subset)](https://github.com/Wisser/Jailer/issues/126) — 2025-08-05. Reverse traversal pulled in rows the subject's WHERE clause excluded. This is the "child expansion blows up the slice" failure, in the most mature tool in the field.
...
- **Language:** Python · **Licence:** MIT · **Created:** 2025-11-02 · **Last commit:** 2026-09-02 · **Stars:** 0.
- Evidence that condenser's abandonment is being felt, but with zero stars and one contributor it is not yet a competitor. Inherits condenser's design, including the absence of masking.
...
- Exports "all the objects needed for a target table to maintain referential integrity" via `COPY` to CSV, with an `import` mode for the other side. `pgsubset --config <CONFIG> --mode <export|import>`.
...
- **Masking:** none.
- **Install:** one-line tarball fetch from GitHub releases. **But it explicitly does not copy the schema** — the documented workflow requires you to `pg_dump --schema-only` and `psql` it into the target first, so the real install path is three commands and a schema round-trip.
...
- **Cycle handling:** "To break a circular dependency, Structural identifies a foreign key column that is NULLable, and sets its values to NULL. When the process reaches a NULL value, it stops looking for additional related records." The same deferral trick Jailer uses, done automatically.
...
- **Licence and price:** proprietary. Third-party pricing research puts cloud-hosted deals at roughly **$15,000–$30,000/year** for one or two data sources ([Vendr](https://www.vendr.com/marketplace/tonicai)) — treat as indicative, not a published list price.
...
- **Licence and price:** proprietary, licensed on a **capacity model priced per TB of production data**, reported at **$9,600 per TB per year** in 1TB increments ([features & editions](https://www.red-gate.com/products/test-data-manager/pricing/), [capacity model](https://www.red-gate.com/support/license/capacity-model/)).
...
- **Install:** Debian/Ubuntu apt repository via Cloudsmith.
...
- **Language:** closed source agent. **Licence:** proprietary; whether any component is open source is **unverified** from the pricing page.
...
| [Basecut](#basecut) | `brew install basecuthq/cli/basecut` | 0 authored (`init` generates ~12) | **~5 min, vendor-claimed** ([quickstart](https://docs.basecut.dev/getting-started/quick-start)) | requires an account and a browser `basecut login` before anything runs |
...
### 1. Nobody tells you which columns are personal — except the two tools that just launched, and one extension
...
- Only **Jailer** and **Tonic Structural** resolve cycles automatically, both by the same trick — defer or null a nullable FK ([Jailer news 2021-02-04](https://github.com/Wisser/Jailer#news), [Tonic docs](https://docs.tonic.ai/app/generation/subsetting/subsetting-about)).
...
- **Jailer shipped a correctness bug from it.** [#126 "Subject condition is violated due to reverse traversal (unexpected rows in subset)"](https://github.com/Wisser/Jailer/issues/126), fixed August 2025 — nineteen years in, the mature tool still leaked rows the subject condition excluded.
...
Deterministic masking is what keeps joins working after the values change. slice-db randomises its pepper on every run by default, so two runs disagree. fixturize has determinism as an [open issue](https://github.com/boringSQL/fixturize/issues/4). Greenmask has a hash engine but [#338](https://github.com/GreenmaskIO/greenmask/issues/338) shows its default company-name generator drawing from an 888-value space with replacement, producing duplicates that "can violate uniqueness constraints". pgEdge Anonymizer is explicit that consistency holds only "within a run".

### 8. The category kills companies

Snaplet [shut down on 31 August 2024](https://supabase.com/blog/snaplet-is-now-open-source) and open-sourced its tools; the team joined Supabase, the snapshot repo is archived, and **`snaplet.dev` no longer resolves in DNS** — the shutdown announcement survives only in the [Wayback Machine](http://web.archive.org/web/20240716025550/https://www.snaplet.dev/post/snaplet-is-shutting-down). Neosync — 4,141 stars, YC-backed — was [acquired by Grow Therapy on 1 August 2025](https://www.crunchbase.com/acquisition/grow-therapy-acquires-neosync-cd81--00632527), archived its repo on 30 August 2025, and `docs.neosync.dev` no longer resolves either. Replibyte had 4,409 stars and a fastest-growing-OSS award and has not merged a commit to `main` since May 2024.

Both dead products' documentation is now unreachable, which means **anyone still running them has no docs**. A tool whose entire behaviour is discoverable from `--help` and a generated config file degrades better than one whose manual lived on a domain the company let lapse.
...
4. **Works offline against localhost and in CI with the same command.** Every tool here claims CI support; only the ones with no control plane actually deliver it unattended.
...
12. **Deterministic masking by default, reproducible across runs.** Same input plus same key produces the same output, so joins on masked natural keys survive. slice-db's random-per-run pepper and [fixturize #4](https://github.com/boringSQL/fixturize/issues/4) show what happens without it.
...
- **Container discovery on first run.** No tool auto-discovers running Postgres containers and proposes a source and target pair. Every quickstart in this document starts with the user pasting two connection strings.
