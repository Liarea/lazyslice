...
| 6 May 2024 | Snaplet Seed launches on Product Hunt, #2 of the day | [Show HN](https://news.ycombinator.com/item?id=40275677), [Product Hunt](https://www.producthunt.com/posts/snaplet-seed) |
...
| ~Nov 2025 | `supabase-community/snapshot` archived (exact archive date not exposed by the GitHub API; last push 5 Nov 2025) | [repo](https://github.com/supabase-community/snapshot) |
...
> I have used Snapplet to seed our dev database with anonymized prod data to run complex data migration, It helped us to test the migration on our dev environment without being scared to mess the prod db It was 🔥
> — Thibault Le Ouay, openstatus ([Product Hunt](https://www.producthunt.com/posts/snaplet-seed))

> That looks exactly like something we would need. I spent hours, days, probably weeks crafting our seed data. 🙈
> — Dominik Hackl, fynk ([Product Hunt](https://www.producthunt.com/posts/snaplet-seed))

> This looks handy, creating scripts to seed data into databases is actually quite time consuming. Especially if I want to make the data realistic.
> — Joonatan, Stockle ([Product Hunt](https://www.producthunt.com/posts/snaplet-seed))
...
> Note that the `percent` / `rowLimit` specified in the subset config may not be exact. The actual row count of the data is affected by the relationships between the tables. **As such, a 5% subset specified against a specific table may ultimately include more than 5% of the actual database.**
...
Thirteen months after the report, a stranger sent the fix: [PR #22, "fix(cli): npm install is broken, need to upgrade better-sqlite3"](https://github.com/supabase-community/snapshot/pull/22), opened 18 November 2025 by faisalburhanudin. It is still open, and it always will be — the repository was archived within days of it landing, so **nobody can merge it**. A one-line dependency bump that would restore the tool for everyone is frozen in place forever.
...
Roughly nine months of a broken seed pipeline at a volunteer-run charity, resolved by renaming the file so the build would stop failing.
...
Nine knobs, several of which require the user to understand graph traversal to set. And the documented way to use one of them:
...
> — patricktyndall ([#3420, "Can't connect to localhost database on Mac"](https://github.com/nucleuscloud/neosync/issues/3420), 27 Mar 2025)
...
Five database backends, two object stores, a Terraform provider, a Python SDK, an LLM integration, RBAC, SSO, audit logs, webhooks, job hooks, Slack — in twenty-one months. Two things are conspicuous:
...
2. **Breadth beat depth.** MongoDB, DynamoDB and SQL Server all shipped in a nine-week window in mid-2024 ([Jun](https://github.com/nucleuscloud/neosync/blob/main/docs/blog/2024-06-20-mongodb.md), [Aug](https://github.com/nucleuscloud/neosync/blob/main/docs/blog/2024-08-01-dynamodb.md), [Aug](https://github.com/nucleuscloud/neosync/blob/main/docs/blog/2024-08-29-sql-server-support.md)), while [#3227](https://github.com/nucleuscloud/neosync/issues/3227) shows the *Postgres* subsetting graph was still directional-only in 2025.
...
> — pistoriusp ([HN, 6 Jan 2026](https://news.ycombinator.com/item?id=46523330))
...
| [snapshot](https://github.com/supabase-community/snapshot) | 5 Nov 2025 | v0.93.2, 2 Aug 2024 | 12 | 9,808 | **Archived** |

(Verified via the GitHub and npm registry APIs on the research date.)

Every contributor to all three repositories is either a former Snaplet employee or a one-commit drive-by. Copycat: justinvdm 160, peterp 12, khaya-zulu 7, avallete 6, then singles. Seed: jgoux 115, justinvdm 92, avallete 57, CarelFdeWaal 18 — nobody else. Snapshot: CarelFdeWaal 30, jgoux 9 ([contributors](https://github.com/supabase-community/snapshot/graphs/contributors)).

There is one genuinely hopeful signal in that table. In November 2025, fifteen months after the shutdown, a former Snaplet engineer spent a day trying to compile `snapshot` into **per-platform binaries** — commits titled "compile binary for each platform", "try to build mac binary on macos M1" ([commits](https://github.com/supabase-community/snapshot/commits/main)). Then the repository was archived. Somebody worked out that the thing standing between this tool and a long life was its Node packaging, and got there too late.
...
Snaplet's subset config reached nine parameters — `percent`, `rowLimit`, `where`, `orderBy`, `keepDisconnectedTables`, `followNullableRelations`, `maxCyclesLoop`, `maxChildrenPerNode`, `taskSortAlgorithm`, `eager`, `traversalMode` — and the docs instructed users to set `maxCyclesLoop` to 0 and *"gradually increment it until the subset of data you fetch trough a relationship is enough for your use case"*, while separately admitting a 5% subset *"may ultimately include more than 5%"*. That is the whole hard problem handed back to the user as homework, plus a headline number that does not mean what it says. lazysnap's plan step must decide these itself, print what it decided in one line of plain language, report the honest resulting row count rather than the requested one, and only then write the decisions into `lazysnap.yml` as a record. *Evidence: [subset docs](https://github.com/snaplet/docs-old/blob/main/docs/04-references/data-operations/04-reduce.md), [final capture docs](https://snaplet-snapshot.netlify.app/snapshot/core-concepts/capture).*
