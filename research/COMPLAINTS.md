...
| PostgreSQL Anonymizer (Dalibo) | Active on GitLab. 3.1.3 released 2026-06-29; "Release 3.2" issue opened 2026-09-04 | [repo](https://gitlab.com/dalibo/postgresql_anonymizer), [issue #674](https://gitlab.com/dalibo/postgresql_anonymizer/-/issues/674) |
...
**FK-7** — [Greenmask issue #403, 2026-02-19](https://github.com/GreenmaskIO/greenmask/issues/403), a two-table repro:
...
**FK-11** — [Greenmask issue #392, 2026-01-16](https://github.com/GreenmaskIO/greenmask/issues/392), the maintainer's own epic **[maintainer]**:
...
**FK-12** — [Greenmask issue #319, 2025-07-21](https://github.com/GreenmaskIO/greenmask/issues/319) **[maintainer]**, summarising what users keep reporting:
...
**FK-13** — [Greenmask issue #270, 2025-02-26](https://github.com/GreenmaskIO/greenmask/issues/270), `mikfreedman`, on relationships that do not point at a primary key:
...
**FK-14** — [Jailer issue #126, 2025-08-05](https://github.com/Wisser/Jailer/issues/126) — the subset pulled in strangers:
...
**FK-15** — [Jailer issue #120, 2025-03-21](https://github.com/Wisser/Jailer/issues/120), `patricktyndall`, on cycle handling emitting untyped NULLs:
...
**FK-16** — [Jailer issue #50, 2021-08-19](https://github.com/Wisser/Jailer/issues/50), `raresboza`, asking whether the advertised cycle handling is real:
...
**FK-17** — [Neosync issue #3227, 2025-02-06](https://github.com/nucleuscloud/neosync/issues/3227), `Yarn-e`:
...
**FK-18** — [Neosync issue #3433, 2025-03-28](https://github.com/nucleuscloud/neosync/issues/3433), `patricktyndall`, on masking a key that other tables point at:
...
**FK-22** — [Xata blog, 2026-01-22](https://xata.io/blog/anonymization-the-missing-link-in-dev-workflows) **[vendor]**:
...
When the maintainer answered that auto-detection of sensitive data and schema-change detection were planned, the reply was: *"Those are great features to have but they're not in the app yet."*
...
**PII-16** — [gonymizer README](https://github.com/smithoss/gonymizer), the tool SmithRx built for HIPAA data, in its own words:
...
**CB-8** — [Snaplet seed issue #205, 2024-12-04](https://github.com/supabase-community/seed/issues/205), on a "quick start" that demanded an OpenAI account:
...
**CB-13** — [Neosync issue #3133, 2025-01-13](https://github.com/nucleuscloud/neosync/issues/3133) **[maintainer]**, filed by the maintainers against their own product:
...
**CB-14** — [Replibyte issue #261, 2023-01-30](https://github.com/Qovery/Replibyte/issues/261) — the config schema itself is the limit:
...
The dbslice maintainer's answer describes the artefact he thinks unlocks that: compliance profiles that "auto-appl[y] masking rules, scan the output for residual PII, and generate an audit manifest your data custodian can review". `patpatpat`'s reply: *"Sounds fantastic."*
...
**TR-14** — [Greenmask issue #465, 2026-07-10](https://github.com/GreenmaskIO/greenmask/issues/465), on `validate` passing and `restore` failing:
...
**TR-15** — [Greenmask issue #469, 2026-07-28](https://github.com/GreenmaskIO/greenmask/issues/469) — a read-only source role producing a silently wrong target:
...
**TR-16** — [dbslice issue #8, 2026-05-20](https://github.com/nabroleonx/dbslice/issues/8) — twenty minutes of extraction, then nothing:
...
**SP-2** — [Replibyte issue #293, 2024-01-30](https://github.com/Qovery/Replibyte/issues/293), restoring 8 million rows / 4 GB:
...
**SP-3** — [Replibyte issue #268, 2023-06-14](https://github.com/Qovery/Replibyte/issues/268), `maxleroy`:
...
**SP-5** — [Replibyte issue #289, 2023-12-24](https://github.com/Qovery/Replibyte/issues/289), on Pagila, a *sample* database:

> the dump takes hours while on linux takes few minutes

**SP-6** — [Jailer issue #93, 2022-08-07](https://github.com/Wisser/Jailer/issues/93):
...
**SP-8** — [PostgreSQL Anonymizer issue #507, 2025-01-20](https://gitlab.com/dalibo/postgresql_anonymizer/-/issues/507):
...
**SP-9** — [PostgreSQL Anonymizer issue #637, 2026-05-13](https://gitlab.com/dalibo/postgresql_anonymizer/-/issues/637), on the cost of doing FK-correct grouping — the price of Theme 1 shows up as Theme 5:
...
**SP-11** — [Neosync issue #3481, 2025-04-09](https://github.com/nucleuscloud/neosync/issues/3481), on time lost to a job that will not say what is wrong:
...
**SP-12** — [Xata blog, 2026-01-22](https://xata.io/blog/anonymization-the-missing-link-in-dev-workflows) **[vendor]**, describing the pipeline it sells against:
...
**DB-5** — [Neosync issue #3411, 2025-03-26](https://github.com/nucleuscloud/neosync/issues/3411), on a minor-version boundary:
...
The founder's answers, in order: "kinda", "no, but I should", "yes … ip4 and ip6", and "again, not yet".
...
**DB-10** — [Greenmask issue #394, 2026-01-29](https://github.com/GreenmaskIO/greenmask/issues/394) — silent data corruption on a plain passthrough column:
...
**DB-12** — [PostgreSQL Anonymizer issue #635, 2026-05-12](https://gitlab.com/dalibo/postgresql_anonymizer/-/issues/635) — the distribution channel lying about what it ships:
...
**DB-13** — [Jailer issue #104, 2023-05-09](https://github.com/Wisser/Jailer/issues/104) — a user asking the best-maintained subsetter in the field whether it also masks:
...
**DB-14** — [Neosync issue #3420, 2025-03-27](https://github.com/nucleuscloud/neosync/issues/3420), on the most ordinary local setup there is:
...
**CI-3** — [Greenmask issue #333, 2025-08-15](https://github.com/GreenmaskIO/greenmask/issues/333) — install friction, offered by a user as a PR:
...
**CI-5** — [Neosync issue #3560, 2025-07-31](https://github.com/nucleuscloud/neosync/issues/3560) — what "install the tool" means when the tool is a platform:
...
- **Trust rises to second on this pass, and structural evidence supports it.** Three of the nine tools in the state table are archived or dead and two more are stale. A developer choosing a tool in 2026 has watched Snaplet close, Neosync be acquired and archived, Replibyte go four years without a release and Datanymizer go quiet. "Why will you still be here in two years" is a fair question to ask a new entrant, and the honest answer is structural: an abandoned static binary keeps working; an abandoned hosted service does not.
- **Silent success is the sharpest sub-theme and it cuts across four themes.** TR-15 (wrong sequences, exit 0), TR-16 (empty output file, exit 0), PII-2 (no transformation, exit 0), PII-4 and PII-5 (parallel masking reports success while leaving cleartext), FK-10 (subset silently returns ~0 rows), DB-10 (leading zeros stripped on a passthrough column). Seven independent reports across five tools in which the tool said it succeeded and had not. If this project makes one guarantee, "the run either verifies itself or fails loudly" is the one with the most evidence behind it.
- **CI is last on count but not on importance.** It is under-represented because most complainants have not cleared Themes 1 to 4. The CI complaints that do exist are cheap to satisfy: a non-zero exit code on validation failure, config discoverable from an env var, a one-line install, no cluster required.
- **Overlaps are flagged, not hidden.** Five sources are counted in two themes each: `onion2k` (PII-1/TR-5), `tjpnz` (PII-7/TR-12), `gregwebs`' 2024 comment (CB-7/TR-4), the GoMask Show HN (FK-23/SP-13) and the Xata post (FK-22/SP-12). Two people also appear twice within one theme from different comments: `imiric` (FK-1, FK-4) and `onion2k` (PII-1, PII-14). Counting each *person* once per theme gives FK 22, TR 14, PII 15, DB 16, CB 16, SP 12, CI 12 — the same ordering apart from PII and DB/CB swapping places in the middle. The top and the bottom are robust; ranks 3 to 5 are within noise of a different filing decision.
...
- **No paid-tool complaints.** Tonic, Delphix, Redgate and Broadcom TDM customers complain in support portals and on Gartner Peer Insights, neither publicly quotable. It is plausible that the paid tier's complaints differ in kind — more "the licence costs six figures", less "the binary won't build".
